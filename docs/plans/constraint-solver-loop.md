...
`scripts/permuter/hill_climber.py::hill_climb` now re-fires `constraint_solver.synthesize` at **every** round, not only round 1. Each round re-extracts a fresh `FunctionContext` from the newly-applied source (`extract_function` already runs per round), so subsequent synthesis runs see updated diagnosis state, mutated var-order, and post-edit `mismatch_regions`. Variants that produce identical source bytes to an earlier attempt are filtered out before reaching the scorer.
...
1. **CLI**: `python -m scripts.permuter.hill_climber --no-loop-constraints` (CLI default is ON).
...
- `scripts/permuter/tests/test_loop_constraints.py` (new, 7 tests).

## Files unchanged (intentionally)

- `scripts/permuter/constraint_solver.py` — no behavior change required. `synthesize()` is idempotent on identical input (its own `seen_sources` already dedups within a single call; the new cross-round dedup is the caller's responsibility).
- `scripts/permuter/beam_search.py` — already loops `synthesize()` per beam state.
