...
- [x] **A1** `function_health.py` — per-fn dossier fusing diagnosis + ceiling + pattern suggestions. Single entry point: "what should I try on this symbol?" *Medium effort* — depends on `decomp_synth/diagnosis.py` (already in RB3) and other helper imports.
...
Each pattern is a new file in `decomp_synth/patterns/`; auto-registered via
`Pattern.__init_subclass__`. Canonical shape: `member_readback.py` (~250
lines). Each agent: write file, run `python -m decomp_synth.pattern_scan
...
1. **Don't touch unrelated `src/` files** — there is in-progress work (see `git status`). Wave A and B only modify `scripts/analysis/` and `decomp_synth/patterns/` and possibly `docs/decomp/patterns/`.
...
| A4 | A4 reclassify_at_limit.py | Sonnet 4.6 | DONE | Schema parity: RB3 missing verdict_reason col (gracefully skipped); decomp_synth symlink→DC3 required overriding build_object/run_objdiff/load_unit_source_map locally; dry-run: 551 AT_LIMIT fns, 20 sampled at 90-99.9% → 20 REOPEN (80% MIXED, 20% REGSWAP_PLUS), 0 KEEP |
