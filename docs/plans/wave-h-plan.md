...
| I4 | system/utl/Cheats permuter sweep | Sonnet | DONE | 2 wins / 5 candidates in ~15min. `CallCheatScript` 99.4%→99.99% (hoist `data->mType` to local `dataType` before `||`-chain — target loads mType speculatively before testing `b1`); `OnMsg(ButtonDownMsg)` 99.6%→100% (goto-done convergence — inner `return 1` was materializing `li r3, 0x1` separately from the fallthrough return). `SetCheatMode` 99.1% AT_LIMIT (4 GPR swap pairs: r19↔r24, r25↔r26, r25↔r29, r19↔r20; stack slots 0x10↔0x18 + 0x14↔0x1c SWAPPED in RebuildKeyCheatsForMode inline; permuter exhausted at fixability 0.23). |
| I2 | system/char/CharEyes permuter sweep | Sonnet | DONE | 1 real win / 12 candidates in 138s wall-clock. `Highlight` 81.8%→82.6% (+0.79pp) via decl reorder (trans/oneframe), comparison flips (`f2 < f1`→`f1 > f2`, `unkd4 != unkc8`→`unkc8 != unkd4`), screen.y split into store+negate. `NextLook`/`Poll`/`UpdateOverlay` near-noise improvements (+0.01pp each). 9 other candidates at plateau with 46 variants/round. `LidTrackAndClamping` (90.6%), `EnforceMinimumTargetDistance` (91.0%), `NextLook` (91.8%) appear to be FPR-regswap or structure-class — not permuter-fixable. Note: CharEyes is NonMatching in objects.json so batch_auto must be run with `--no-db-sync` + manual DB population from live objdiff scores. |
