...
- [x] **Port relevant slash commands** from `dc3-decomp/.claude/skills/`:
  ...
  - **Tier 2** — `vtable`, `resolve-vcall`. Planning complete 2026-05-11:
    - [docs/plans/vtable-skill.md](plans/vtable-skill.md) — recommends parsing `.o` relocations from `build/SZBE69_B8/obj/` via pyelftools (NOT Ghidra MCP — Ghidra's loaded ELF is Bank 5 Debug, but build target is Bank 8 SZBE69_B8, and vtable layouts differ between banks: `__vt__9Character` is 604 vs 628 bytes). Empirical findings: MWCC emits one `__vt__N<class>` symbol per class spanning all sub-object vtables; sub-object boundaries marked by repeated `__RTTI__N<class>` relocations; `@<delta>@` thunk symbols name `this`-adjusting thunks. No ICF disambiguation needed (Wii MWCC doesn't fold).
    - [docs/plans/resolve-vcall-skill.md](plans/resolve-vcall-skill.md) — recommends `resolve` as a subcommand of the same `dump_vtable.py` script (cache sharing). **Pre-implementation revision required** (see §0 of that doc): the original plan read vtable bytes from the Bank 5 debug ELF, but should be switched to Bank 8 `.o` relocations to match what objdiff sees, only using DWARF for class-hierarchy enrichment.
    - Implementation estimate: ~half a day to a day total for both skills (shared script, ~250-350 lines).
