#!/usr/bin/env python3
"""ADVERSARIAL-VERIFY-ONLY driver (NOT committed to land): boot the worktree
rb3-native, optionally set a venue override + different song, drive to a deep
gameplay anchor, and let the RB3_SBVERIFY diagnostic in ScrollbarDisplay log
whether any scrollbar mesh draws while a game is active.

Thin wrapper around keyboard-to-gameplay.py's nav primitives but: (1) uses the
WORKTREE binary, (2) injects {meta_performer set_venue_override <v>} before/while
crossing menus, (3) varies --song-downs, (4) holds gameplay for a deep anchor.
"""
...
def main():
    ...
    ap.add_argument("--hold-s", type=float, default=40.0)
    ap.add_argument("--log", default="/tmp/sbv_drive.log")
    ap.add_argument("--out", default="/tmp/sbv_drive")
    args = ap.parse_args()
    ...
    try:
        ...
        while time.time() < dl:
            ...
            hh = kg.health(port)
            if hh is None: break
            # Capture a screenshot at deep anchors (songMs > 30000) for the
            # highway-clean visual check.
