...
| **A4 content downscale** | **OPEN, needs planning** | The *only* remaining 1.5 Mbps lever (that arm is throughput-bound; bytes already saturate the link). **Opus feasibility probe (2026-06-23):** separate texture files are negligible (7 files, 0.2 MB) — **textures live INSIDE the milos**, so A4 must be milo-container-aware, not a file-level recompress. The journey venue `small_club_01.milo_xbox` is **18.5 MB raw / ≈11.7 MB q11 wire** (the W5 `maxSingleMilo`); the 65 small_club venue milos total 205 MB raw. So the lever is downscaling/recompressing the RndTex bitmaps inside the venue (and loaded char/outfit) milos, with a visual quality gate, then re-running q11. Real pipeline work — **needs a Fable plan** (rejected-alternatives + quality-gate design) before implementation. `research/08 §A4` has the earlier sketch. |
