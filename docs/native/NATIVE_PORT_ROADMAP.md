...
**Status**: Phase 0 **COMPLETE**. Phase 1 milestone (b) **COMPLETE**; critical-path **Step 1 nearly complete** — all `rndobj`/`synth` matched-fork TUs are clang-LP64-clean (K2 resolved), `rb3-native` links them, and `RB3_LIVE_LOAD=1` instantiates real objects via `DirLoader`. Default header dump covers **4453/4455 milos**. **Next: finish full object-graph load via the config type-defs (rolls into headless DTA boot) → rendering** — see [Critical path to a booting RB3](#critical-path-to-a-booting-rb3).
