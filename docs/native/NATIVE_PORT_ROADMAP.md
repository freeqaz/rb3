...
| C6 | **char-customize previews** (chars.milo + CharSync Poll) | 🟢 CLOSET REACHED — char renders standing (DEFAULT-ON); anim + head-deform OPEN | Closet (`customize_clothing_screen`) reached without sign-in and renders a standing character (verified, `/tmp/rb3-closet*`). Achieved by C11 (gate flips) + C13 (body) + the sRawCollide crash fix (`ed9a3e92`). NOTE (per the 3-trace+synthesis workflow 2026-06-09): there is **no 4-up customize view** in RB3 — both closet AND manage-band show exactly ONE char (`goto_shot idle_1.shot`); true "4 members standing" is gameplay/venue only. OPEN: the closet preview char is `skinned=0` (static pose, not `Character::Poll`'d into a live skeleton → no idle anim; the motion seen is the orbiting camera) + head deform (C7/C8). |
...
| C11 | **native sign-in / profile gate** (customize requires `CanSaveData`; sign-in is a stubbed redirect-back) | 🟢 guest-profile DEFAULT-ON, cascade RESOLVED; real sign-in still TODO | Landed: faithful offline `TheServer` (`rb3_server_native.cpp`) + guest-profile (`rb3_guestprofile_native.cpp`) **NOW DEFAULT-ON** (opt-out `RB3_NO_GUEST_PROFILE`). The feared "cascade" was root-caused: ①`MainHubPanel`→`TheServer` ✅fixed; ②the "`PropSync<RndTex>`" theory was a RED HERRING — the real crash was `BandPatchMesh::ProjectPatches` writing `RndMesh::sRawCollide` into READ-ONLY `.text` (missing static def → weak no-op-stub alias) when a tattooed char composited → ✅fixed (`ed9a3e92`, real `.bss` defs). Gate predicates flipped via REAL engine APIs: `ProfileMgr::UpdatePrimaryProfile()` (→`has_primary_profile=1`) + `PrefabMgr::OnPrefabToggleCustomizable` (→`is_char_customizable=1`). VERIFIED: clean-env boot has both gates=1, no crash, **gameplay still works**. **Deeper work (TODO)**: real native sign-in via decomp (`PlatformMgr_Wii`/`WiiProfileMgr`) or reimpl, to retire the guest-profile hack. |
| C12 | customize closet reachability | ✅ DONE (default-on) | `PrefabMgr::OnPrefabToggleCustomizable` (HX_NATIVE default-on, HACK/TODO, in the guest-profile install) makes the default prefab char customizable → `customize_character.btn` routes to the closet; `CustomizePanel::Load`'s `GetProfileForUser` is non-null via `UpdatePrimaryProfile`. Empirically reaches `customize_clothing_screen` + renders a char, no crash. |
