...
| C13 | **band-char preview BODY source** — ✅ GATE GREEN, full enable in progress | 🟢 Stage-1 + gate verified (`commit`), Stage 2/3 next | `player0..3` are milo **proxies** (`mProxyFile="../../char/main/main.milo"`); `Dir::PostLoad` loads `main.milo` in → binds `mFileMerger` (FileMerger.fm + outfit + IK + body_clips) → FileMerger loads 13 bodyparts. **Same path gameplay uses** (`acd9c19a`). **VERIFIED in the real App boot** (opt-in `RB3_CHAR_PREVIEW=1` Stage-1 load in `CharCache::InitMe`): all 4 `C13_PROBE: playerN char=<nonnull> FileMerger.fm=<nonnull>`, no crash → `StartLoad`'s `mFileMerger` deref is safe; "bodyless shells" retired; Xbox/Wii mismatch = red herring. (In-process gtest retired — headless can't run the GPU `Rnd::Init` rndobj-factory cluster; verified in the real boot instead.) **Next (workflow)**: Stage 2 harden derefs + Stage 3 un-gate `UpdateCharCache` → load body + animate; headless body/anim probe decoupled from the guest-profile cascade. See [`CUSTOMIZE_PREVIEW_FINDINGS_2026-06-09.md`](CUSTOMIZE_PREVIEW_FINDINGS_2026-06-09.md). |
