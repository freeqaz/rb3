...
| C6 | **char-customize previews** (chars.milo + CharSync Poll) | 🟢 body-load DONE (opt-in); closet-UI + anim gated on C11 cascade | Stage 1/2/3 landed + verified (C13): preview chars load full bodies. Remaining for the on-screen closet: C11 cascade (reach the UI) → the closet milo-proxy Polls the chars → skeleton binds + animates. |
