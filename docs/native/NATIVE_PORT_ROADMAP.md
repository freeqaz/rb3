...
| C6 | **char-customize previews** (chars.milo + CharSync Poll) | 🟢 char RENDERS + ANIMATES in the closet (OPT-IN); default-on blocked on C11 domino ②; head-deform = C7/C8 | With `RB3_GUEST_PROFILE=1 RB3_CHAR_PREVIEW=1` the closet (`customize_clothing_screen`) is reached without sign-in and shows a **standing, SKINNED, ANIMATING** character (`92fcb32c`). Skinning: engine `SKIN_PROBE` = **81 SKINNED-PATH meshes** (the old `skinned=0` was a probe-traversal artifact; `rb3_char_probe`/`SKEL_REBIND` miss the FileMerger-merged drawn meshes — the char was always skinned). Animation: `ClosetMgr` Enter()+`SetContext("venue")`+`SetState("stand",0x1000)` loops an idle (`BAND_ANIM`: `player0_m` clip plays, bone worldPos changes across frames). NO 4-up customize view exists (both closet + manage-band show ONE char); true 4-members = gameplay/venue. OPEN: default-on (web) blocked on C11 domino ②; head deform = C7/C8; idle uses the venue context (polish: wire closet `clips`). |
| C7 | BandHeadShaper head-milo load | ✅ not the cause (dead natively) | The gameplay head shard was NOT BandHeadShaper (provably inert: `BandHeadShaper::Init` forces `gHeadMale`/`gHeadFemale` load false, `Start` returns false on empty mapping) nor gDeforms (one-shot CPU bake) — it was the C8 skinning bind. BandHeadShaper head-milo morph remains a separate (cosmetic, non-shard) deferral. |
| C8 | char head/hands/face rebind residual | ✅ FIXED (`0de768a1`, default-on) | The head/hair/hands/face skin meshes sharded in gameplay (white radiating spikes on closeups) because they bound by name to the STATIC male-bind `skeleton.milo` magnet; the authored inverse-bind vs magnet basis error explodes thin geometry (R·sin θ). Fix: `BandCharacter::RebindHeadHandsAtRest()` (Poll, BEFORE `Character::Poll`) snapshots each per-member bone's REST WorldXfm (first Poll = SetDeformation gender-bind rest pose) and bakes `mOffset = meshWorld·inverse(restWorld)`, binding to the LIVE per-member bone → coherent AND animating (CharHair/CharFaceServo drive the same per-member skeleton). Verified: REBIND_DRAW_FLING=0, SKINPOS 37-65u, closeups coherent, torso/crowd untouched, no crash. Opt-out `RB3_NO_HEAD_REBIND=1`. HX_NATIVE-only. Diagnosed via 8-agent ultracode workflow. See [`CHAR_GAMEPLAY_WORK_2026-06-09.md`](CHAR_GAMEPLAY_WORK_2026-06-09.md). |
...
| C11 | **native sign-in / profile gate** (customize requires `CanSaveData`; sign-in is a stubbed redirect-back) | 🟡 guest-profile OPT-IN (works); default-on blocked on domino ② (`PropSync<RndTex>`); real sign-in TODO | Landed: faithful offline `TheServer` (`rb3_server_native.cpp`) + guest-profile (`rb3_guestprofile_native.cpp`, **OPT-IN** `RB3_GUEST_PROFILE=1`). Gate flipped via `PrefabMgr::OnPrefabToggleCustomizable` (→`is_char_customizable=1`, opens the closet). Two real crashes found+fixed along the way: ①`MainHubPanel`→`TheServer` ✅; the `sRawCollide` read-only-`.text` write in `ProjectPatches` ✅ (`ed9a3e92`). **Default-on REVERTED** (`92fcb32c`): guest+char-preview BOTH on → `song_select` SIGSEGV = the real **domino ②** — a menu DTA `RndMat::Handle`→`OnSet`→`SetProperty`→`RndMat::SyncProperty`→`PropSync<RndTex>` (PropSync_p.h:124) `dynamic_cast` on a **dangling `node.GetObj()`** (freed Hmx::Object → null-vtable fault). Each flag alone is safe; default OFF is verified clean (3/3 game_screen). **TODO**: (a) fix the dangling-DataNode crash → restore default-on (web); (b) real native sign-in (decomp `PlatformMgr_Wii`/`WiiProfileMgr` or reimpl) to retire the guest hack. (`UpdatePrimaryProfile` was dropped — it made the hollow guest profile primary, feeding `CharSync` garbage StandIns.) |
| C12 | customize closet reachability | ✅ DONE (opt-in) | `PrefabMgr::OnPrefabToggleCustomizable` (in the opt-in guest-profile install) makes the default prefab char customizable → `customize_character.btn` routes to the closet. Empirically reaches `customize_clothing_screen` + renders + animates a char, no crash (with `RB3_GUEST_PROFILE=1 RB3_CHAR_PREVIEW=1`). Does NOT need `UpdatePrimaryProfile` (dropped — caused the C11 domino ②). |
| C13 | **band-char preview BODY source** — ✅ DONE (body loads natively) | 🟢 Stage 1/2/3 landed + verified | `player0..3` are milo **proxies** (`mProxyFile="../../char/main/main.milo"`); `Dir::PostLoad` loads `main.milo` in → binds `mFileMerger` → FileMerger loads 13 bodyparts (same path gameplay uses, `acd9c19a`). **VERIFIED in the real boot** (opt-in `RB3_CHAR_PREVIEW=1`): gate `FileMerger.fm` bound for all 4; Stage 2/3 (`UpdateCharCache` un-gate + deref hardening) → `{rb3_char_probe N}` = **`meshes=140 verts=15395 loading=0`** for all 4, NO crash (no prefab asserts). "bodyless shells" retired; Xbox/Wii mismatch = red herring. `skinned=0` pre-Poll (skeleton binds at `Character::Poll`, which the closet UI supplies via the proxy). **Animation** can't be driven by a headless poke (`SetContext`+`Poll` raw SIGSEGVs — needs `ClosetMgr::SetUser`/`PreviewCharacter` setup); it happens for free once the **closet UI** is reachable (→ C11 cascade). (In-process gtest retired — headless can't run the GPU `Rnd::Init` factory cluster.) See [`CUSTOMIZE_PREVIEW_FINDINGS_2026-06-09.md`](CUSTOMIZE_PREVIEW_FINDINGS_2026-06-09.md). |
