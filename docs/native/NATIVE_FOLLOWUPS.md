...
- **GAP 4 festival crowd backdrop — Option B BLOCKED on missing assets** (`docs/native/c8-bink-2026-06-30/`): Option A (above) killed the white flash. Option B (decode the movie into the crowd RT) is render-code-bounded (~1 day, RB3-only/DC3-safe — wire the **existing** `FFmpegMovieImpl`/`WebMovieImpl` to the `TexMovie` RT) BUT is **hard-blocked on the source `.bik` assets**: `festival_01.milo` references `fest1_mass*.bik`/`mass_crowd*.bik`, but those files were never extracted into `orig-assets` (other venues only have the intro stingers `crowd_*_intro.bik`, not the in-world mass loops). Nothing to decode → not testable. Unblock = obtain the mass-crowd `.bik`s from a fuller disc dump, THEN implement Option B1 (`native/src/rb3_texmovie_native.cpp` + `TexMovie::DrawToTexture` HX_NATIVE branch).
