...
- **GAP 4 festival crowd backdrop — Option B BLOCKED on disc-only assets (DEFINITIVE, `docs/native/bink-albumart-2026-06-30/bink-unblock.md`):** Option A (above) killed the white flash. Option B (decode the movie into the crowd RT) is render-code-bounded (~1 day, RB3-only/DC3-safe — wire the **existing** `FFmpegMovieImpl`/`WebMovieImpl` to the `TexMovie` RT) but is hard-blocked: the festival jumbotron biks (`fest{1,2}_mass*.bik`) the milos reference are **loose Xbox/PS3 disc-streaming files that were NEVER packed into the main ARK on EITHER platform** — exhaustively confirmed (Wii `.wbfs` ARK header = 0 refs; full `arkhelper` extraction of `main_xbox.ark` = only 6 biks, none mass-crowd; filesystem-wide find = 0). We have the ARK + the Wii wbfs but NOT a full 360/PS3 disc-filesystem image. **Unblock = obtain a full RB3 Xbox 360 (or PS3) disc image** and pull the loose `fest{1,2}_mass*.bik`, THEN implement Option B1 (`native/src/rb3_texmovie_native.cpp` + `TexMovie::DrawToTexture` HX_NATIVE branch).
