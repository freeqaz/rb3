...
- **Crowd/extras mesh-shard sub-part (GAP B(b)) — routed out:** a few accessory shards (eyebrows/hair/shoe-skin) explode and are ALREADY correctly guard-dropped (SHARD_GUARD doing its job; forcing them on shifts crowd white% by ≤3%). Faithful fix = a crowd/extras **servo-skeleton rest-rebake** (same family as `RebindInstStringsToRestBasis`) — its own workstream, cf [[project_char_skinning_deform]]. The *visible* big_club white crowd itself is FIXED above.
