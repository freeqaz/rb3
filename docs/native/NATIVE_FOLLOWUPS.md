...
- **GAP 4 festival crowd backdrop — Option B (native Bink/`TexMovie` decoder) OPEN:** Option A (above) killed the white flash; rendering the *animated crowd backdrop* needs a real in-world movie decoder (large, festival-only payoff). Deferred.
...
- **The real gate to reopening the thin-geometry family (footwear + crowd accessories) is the C8 rotation-basis fix** — cf [[project_dc3_feet_in_floor_anim]] / [[project_char_skinning_deform]]; a major char-anim frontier, not a convergence-batch item.
