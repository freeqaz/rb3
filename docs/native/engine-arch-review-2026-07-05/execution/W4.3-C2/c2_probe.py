#!/usr/bin/env python3
"""C2c RankTier probe capture. Boots the isolated W4.3-C2 build with
RB3_RANKTIER_LOG=1, navigates to song_select on '25 or 6 to 4', captures the
sidebar + partdiff, and the [RANKTIER] log lines land in the engine log."""
...
def main():
    ...
    try:
        ...
        time.sleep(2.5)
        screenshot(port, "ss_list.png")
        for _ in range(3):
            http_post(port, "/api/input", "down"); time.sleep(0.25)
        time.sleep(2.5)
        screenshot(port, "ss_diffpanel.png")
        # part difficulty
        http_post(port, "/api/input", "msg:music_library:select_highlighted_node"); time.sleep(0.5)
        if wait_for(port, lambda f,m,s: s=="part_difficulty_screen", 30, "partdiff", proc):
            time.sleep(3.0); screenshot(port, "partdiff.png")
        return 0
    finally:
        try:
        ...
        logf.close()
        # echo the RANKTIER lines
        try:
            with open(log_path) as f:
                lines = [l for l in f if "[RANKTIER]" in l]
            print(f"[c2] === {len(lines)} RANKTIER lines ===", flush=True)
            for l in lines[:120]: print(l.rstrip(), flush=True)
            with open(os.path.join(OUT, "ranktier.log"), "w") as f: f.writelines(lines)
        except Exception as e:
            print(f"[c2] logdump failed: {e}", flush=True)
