...
| **R5** | **Hands endgame decision** | **DECIDED Wave 17 — verdict C** (`execution/R5-HANDS-ENDGAME/VERDICT.md`, rb3 `fd2f83ac`): D4's headline dissolved (clip curl-coverage, not skeleton); native skeleton statics externally exonerated ≤0.65° on comparable channels; fix path = pre-registered branch table (GT-A palette forensics prior / GT-B anim-basis / GT-C decode / GT-D closure pre-authorized) D5 + VISCAP both exhausted (nav wall SOLVED; substrate never animates band CharBones — 0/992 in live gameplay) → **GT-D CLOSURE executed Wave 18** (`execution/R5-HANDS-ENDGAME/CLOSURE.md`, honest no-articulated-ground-truth label, reopen condition pre-registered; mitten mitigation flag-first, E1-decided). §8 correction: `RB3_NO_SKIN_CLAMP` does NOT touch band hands. | The flagship remaining visual bug | CLOSED (mitten polish pending E1) |
