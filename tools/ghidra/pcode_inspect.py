...
Usage:
    pcode_inspect.py "Poll__9CharacterFv"
    pcode_inspect.py "Character::Poll"
    pcode_inspect.py "0x80123456"
    pcode_inspect.py "Poll__9CharacterFv" --switches
    pcode_inspect.py "Poll__9CharacterFv" --casts
...
def resolve_function(client: MCPClient, name_or_addr: str, verbose: bool = True) -> tuple:
    ...
    for sym in symbols:
        ...
        # Detect thunk/vtable/RTTI entries (CW mangling style)
        is_thunk = (
            "thunk" in sym_name.lower() or
            "__vt__" in sym_name or       # CW vtable prefix
            "__RTTI__" in sym_name or      # CW RTTI prefix
            sym_name.startswith("@") or    # CW thunk prefix
            "__ti__" in sym_name           # CW typeinfo prefix
        )
        ...
        # Exact match on name portion
        if query_lower in sym_name.lower():
            score += 10

        # STRONGLY prefer non-thunk functions
        if is_thunk:
            score -= 50
        ...
        # Prefer CW mangled names with proper suffix (Fv, Fi, etc.)
        if "__" in sym_name and sym_name.endswith("Fv"):
            score += 2
...
def main():
    parser = argparse.ArgumentParser(
        description="Inspect Ghidra decompilation for switch statements and cast operations (RB3)."
    )
    parser.add_argument(
        "function",
        help='Function name (e.g. "Poll__9CharacterFv", "Character::Poll") or address (e.g. "0x80123456")',
    )
    parser.add_argument(
    ...
    if not args.no_decompile:
        try:
            ...
            if isinstance(result, dict):
                ...
                # Try to extract address from the name field
