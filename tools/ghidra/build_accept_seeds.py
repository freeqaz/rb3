...
identities, with two anti-leak / data-hygiene filters:
  ...
  1. ANTI-LEAK: drop any ACCEPT entry whose xenon_addr is in the original
     ...
     leak-free. (expected: exactly 73 drops — 47 BSIM / 24 ExactInstr / 2 Implied)

  2. STRICT 1:1: ghidriff seeds are pre-accepted match pairs; a duplicated p1
...
def main() -> int:
    ...
    print(f"ACCEPT in holdout (dropped, anti-leak): {len(drops)}")
    drop_types = Counter(e["match_types"][0] for e in drops)
    print(f"  drop match_types[0]: {dict(drop_types)}")
    assert len(drops) == EXPECT_HOLDOUT_DROPS, \
        f"expected {EXPECT_HOLDOUT_DROPS} holdout drops, got {len(drops)}"
