...
- **`tools/ninja-locked`** — flock wrapper that serializes builds (ninja requires one process per build dir). Wired into `objdiff.json`'s `custom_make` and a `~/.zshrc` `ninja()` function. **Build with this, not bare `ninja`** — concurrent runs otherwise silently corrupt `.o` files and race on `build.ninja`. The lock is **per build directory**, derived from the directory ninja will actually build in (cwd, honouring `-C`, canonicalised with `pwd -P`) — *not* from the script's own location. It is deliberately **not** a global mutex: separate worktrees have separate build dirs and must build in parallel at full speed.
- **No binary deps cache** — `deps="gcc"` was removed from all build rules, so ninja reads `.d` files directly. This kills the rebuild-everything failure mode and survives killed builds, but does *not* make concurrent builds safe (that's the wrapper's job). Verified 2026-08-04: SIGKILL mid-build, then rebuild → 142 done + 590 remaining = 732 exactly, no rebuild-everything.
- **Repo-relative depfiles** — `tools/upstream/transform_dep.py` emits `.d` prerequisites **relative** to the build root. This is load-bearing *because* `deps=` is off: ninja reads the `.d` text directly and resolves it against its own cwd, so a relative prerequisite means "this tree's copy". When these paths were absolute, `setup-worktree.sh`'s reflinked cache pointed every worktree's header deps back at **main**, and a header edit in a worktree invalidated nothing — ninja said "no work to do" and objdiff returned a **stale, identical** result. That failed *silently* (reported no change, not an error). `tools/normalize-depfiles.py --check` verifies any tree and exits nonzero if unsafe; worktree setup runs it and hard-fails. See [docs/decomp/worktree-setup.md](docs/decomp/worktree-setup.md).
...
- **Never use `ninja -n` as an invalidation check — use `tools/ninja-dry`.** When `build.ninja` is itself out of date (ordinary decomp work touches `config/SZBE69_B8/objects.json`, one of its inputs; a fresh worktree is stale by construction), ninja regenerates the manifest first and then, in dry-run mode, **deliberately `exit(0)`s**. It prints `[1/1] RUN configure.py` and returns **0** having never looked at a compile edge. Measured: with `objects.json` + `src/system/obj/Object.h` touched, `ninja -n` reported 1 edge / exit 0 while **727 edges (724 TUs)** were genuinely pending — output *identical* to the case where nothing is pending. A dry run never writes `.ninja_log`, so repeating it never clears the state. `tools/ninja-dry` materialises the manifest first, asserts convergence, and reports `CLEAN`/`PENDING n/N` (exit 0/1) or refuses to answer (exit 2) — it never reports an empty result as a pass. **Rule of thumb: if `ninja -n` prints `RUN configure.py`, the answer you are reading is void.** See [docs/decomp/ninja-dry-run-false-negative.md](docs/decomp/ninja-dry-run-false-negative.md).
