...
Examples:
  ...
  python3 dc3_compare.py --filter system/char/CharBones --dc3-src

  # Show both source paths
  python3 dc3_compare.py --filter system/char/ --dc3-src --rb3-src

  # Output just source file paths (for xargs/editor integration)
  python3 dc3_compare.py --filter system/char/CharBones --paths dc3

  # Only functions of a certain size range
  python3 dc3_compare.py --filter system/char/ --min-size 64 --max-size 256

  # Export results as JSON
  python3 dc3_compare.py --filter system/char/ --export json

  # Export results as CSV
  python3 dc3_compare.py --filter system/char/ --export csv
""",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Use demangled-name matching (catches C++ funcs with different mangling) (default: on)",
    )
    parser.add_argument(
