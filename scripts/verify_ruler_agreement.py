...
section, anonymous `lbl_*` labels -- and our per-TU mwcc obj do not
...
symlink shared with ../rb3-xenon and ../dc3-decomp, so all three repos are
exposed and the fix is config-only in each.

The finding originated in dc3-decomp (Dance Central 3, MSVC/Xbox 360), where it
was measured at 155 functions / 120,728 bytes.  It transfers in full to this
repo despite the different toolchain: measured whole-binary here (rb3, Wii/mwcc)
on 2026-08-31, **151 functions / 224,892 bytes** scored LOWER through `diff`
than through `report generate`, and **zero** scored higher; max delta 7.50 pp;
`ppc.calculatePoolRelocations` alone explains 151/151.  Two of them (284 B) read
exactly 100.0 in report.json and <100 through `diff`.  ../rb3-xenon measures
102 functions / 55,604 bytes.
...
WITNESS_UNITS: tuple[str, ...] = (
    "main/sdk/ec/src/ec_asyncOp",
    "main/App",
    "main/band3/tour/TourDescPanel",
    "main/band3/net_band/RockCentral",
    "main/band3/meta_band/BandStorePanel",
    "main/system/os/HolmesClient",
    "main/system/os/System",
    "main/system/utl/CacheMgr_Wii",
    "main/system/bandobj/BandCharacter",
    "main/system/meta/StorePackedMetadata",
    "main/sdk/RVL_SDK/src/vi/vi3in1",
    "main/sdk/RVL_SDK/src/usbmic/usbmic",
    "main/band3/meta_band/BandSongMgr",
)
...
def check_config(repo: Path, pins_only: bool = False, stamp_out: str | None = None) -> int:
    ...
    if problems:
        ...
        print("\n".join(problems))
        print(
            "\nFix in the `options` block of tools/upstream/project.py (NOT by passing "
            "-c at each call site), then re-run configure.py."
        )
        return 1
