...
// --prewarm on|off : 'off' passes RB3_TEX_PREWARM_OFF=1 (baseline), 'on' = default (fix).
const PREWARM = (process.argv[process.argv.indexOf('--prewarm') + 1]) || 'on';
const ENV_EXTRA = (PREWARM === 'off') ? ';RB3_TEX_PREWARM_OFF=1' : '';
const OUT = `/home/free/code/milohax/rb3/docs/native/frame-stall-2026-06-20/cap/texprewarm-${PREWARM}`;
...
await page.goto(`http://127.0.0.1:${PORT}/?debug=true&env=${encodeURIComponent('RB3_FRAME_TRACE=/trace.jsonl' + ENV_EXTRA)}`, { waitUntil: 'domcontentloaded' });
...
console.log(`\n==== TICK PROBE (gameplay) — prewarm=${PREWARM} ====`);
