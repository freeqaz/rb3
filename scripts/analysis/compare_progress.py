...
Examples:
    ...
    python3 scripts/analysis/compare_progress.py baseline.json build/SZBE69_B8/report.json

    # Show detailed unit breakdown
    python3 scripts/analysis/compare_progress.py --detailed baseline.json current.json
    ...
    python3 scripts/analysis/compare_progress.py --snapshot --filter 'system/*'
...
# Subsystems to exclude by default (third-party, SDK, tiny standalone files)
EXCLUDED_PREFIXES = ("main/sdk", "main/lib")
DEFAULT_MIN_SIZE = 10240  # 10KB


# Default map file for symbol resolution
SCRIPT_DIR = Path(__file__).parent
PROJECT_ROOT = SCRIPT_DIR.parent.parent
DEFAULT_MAP_FILE = PROJECT_ROOT / "orig" / "SZBE69_B8" / "files" / "band_r_wii.map"
...
def get_subsystem(name: str) -> str:
    ...
    RB3 units look like: main/system/char/Char, main/band3/game/GemPlayer
    We want: system/char, band3/game, etc.
    """
    parts = name.split("/")
    if len(parts) >= 3:
...
def expand_filter_patterns(patterns: list) -> list:
    ...
    prepend '*/' so 'system/ui/*' matches 'main/system/ui/Foo'.
    """
    expanded = []
    for p in patterns:
        expanded.append(p)
...
def get_category(subsystem: str) -> str:
    """Categorize a subsystem for RB3."""
    if subsystem.startswith("sdk/") or subsystem.startswith("SDK/"):
        return "SDK"
    if subsystem.startswith("lib/"):
        return "Third-Party"
    if subsystem.startswith("band3/") or subsystem.startswith("network/"):
        return "Game Code"
    if subsystem.startswith("system/"):
        return "Milo Engine"
    return "Standalone"
...
def print_overview(report: dict):
    ...
    print()
    print(f"{'='*70}")
    print(f"  RB3 DECOMP OVERVIEW: {total_pct:.2f}% fuzzy match")
    print(f"  Code: {fmt_bytes_plain(total_code)}")
    print(f"  Functions: {total_funcs_matched:,} / {total_funcs:,}")
    print(f"{'='*70}")

    categories = ["Game Code", "Milo Engine", "Third-Party", "SDK", "Standalone"]
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare RB3 decomp progress between two report.json files, or show snapshot",
        epilog="""examples:
  %(prog)s --snapshot                           Show all subsystems
  %(prog)s --snapshot --filter 'system/ui/*'    Show only system/ui subsystems
  %(prog)s -g '*/char/*' -g '*/band3/*'        Multiple filters
  %(prog)s -g 'system/*' --functions baseline.json current.json
  %(prog)s --overview --filter 'band3/*'        Overview filtered to game code
""",
    ...
    parser.add_argument(
        "--overview", "-o",
        action="store_true",
        help="Show complete overview grouped by category (Game/Milo/SDK)",
    )
    parser.add_argument(
        "--snapshot", "-s",
        action="store_true",
    ...
    parser.add_argument(
        "--all", "-a",
        action="store_true",
        help="Show all subsystems (including sdk, lib, tiny ones)",
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
    ...
    # Overview mode
    if args.overview:
        report_path = args.baseline if args.baseline else DEFAULT_REPORT
