...
# W0.2.S4: three independent stub-set registries, all generated by the same
# gen_band3_link_stubs.py (--set band3|dta|rndobj_synth) and all wired into one
# process-wide census (rb3_stub_census.cpp). A hit is classified by whichever
# registry defines that symbol — the three sets are disjoint (different weak
# symbols entirely), so a plain dict union is safe.
REGISTRIES = [
    os.path.join(REPO, "native", "src", "band3_stub_registry.tsv"),
    os.path.join(REPO, "native", "src", "dta_stub_registry.tsv"),
    os.path.join(REPO, "native", "src", "rndobj_synth_stub_registry.tsv"),
]
REGISTRY = REGISTRIES[0]  # back-compat alias (band3 only) — do not use for classification
...
def load_registry_classes(paths):
    """symbol -> class ('assert-unreachable' | 'ok-noop' | 'data-blob'), merged
    across one or more registry TSVs (a single path is also accepted for
    back-compat). The three W0.2 registries are disjoint symbol sets, so a
    plain last-writer-wins dict union is safe; asserts if that ever changes."""
    ...
    for path in paths:
        with open(path, encoding="utf-8") as f:
            for line in f:
                ...
                sym = parts[0]
                if sym in classes:
                    log(f"WARN: {sym} appears in more than one registry (last one wins: {path})")
                classes[sym] = parts[2]
    return classes
...
# ---------------------------------------------------------------------------
def check_sync():
    log("regenerating band3/dta/rndobj_synth link stubs + census tables from their "
        "registries (drift gate, --all)")
    rc = subprocess.call([sys.executable, GENERATOR, "--all", "--check"])
    if rc == 0:
        log("PASS: no drift — all 3 sets' committed .s/.inc match their registries.")
    else:
        log("FAIL: one or more of {band3,dta,rndobj_synth}_link_stubs.s / "
            "*_stub_table.inc have drifted from their registry. Re-run: "
            "python3 scripts/native/gen_band3_link_stubs.py --all")
    return rc
...
def boot_smoke(args):
    ...
    classes = load_registry_classes(REGISTRIES)
