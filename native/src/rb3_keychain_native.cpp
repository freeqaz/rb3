// rb3_keychain_native.cpp — native KeyChain implementation for the RB3
// (clang LP64) build. Provides .mogg decryption keys for VorbisReader /
// BinkIntegration / Synth::PlayMogg paths.
//
// Why this exists, not the engine's Keygen_Stub.cpp:
//   The engine's KeyChain (milo-native-engine/src/platform/Keygen_Stub.cpp) is
//   a DC3-shaped *namespace* (`namespace KeyChain { ... }`) with
//   `unsigned int getNumKeys()` / `unsigned int` keyIndex. RB3's KeyChain
//   (rb3/src/KeyChain.h) is a *class* with `static int getNumKeys()` /
//   `int` keyIndex. The two mangle differently, so the engine file is
//   correctly excluded for RB3 via MILO_ENGINE_DECOMP_PLATFORM_EXCLUDE in
//   rb3/native/CMakeLists.txt — this file fills the gap.
//
// The matched-fork `src/keygen_wii.cpp` would provide these symbols too, but
// it lives outside the globbed `src/system/**` source set and is not pulled
// into the native build, so RB3 was previously linking the weak no-op stubs
// from `rb3/native/src/rndobj_synth_link_stubs.s`. Those stubs returned 0
// from `getNumKeys()`, tripping the
// `MILO_ASSERT_RANGE(keyIndex, 0, KeyChain::getNumKeys())` in
// `system/synth/VorbisReader.cpp:638` and aborting the mogg load.
//
// Algorithm + key table are copied verbatim from
// milo-native-engine/src/platform/Keygen_Stub.cpp (which in turn derives from
// the original keygen_xbox.cpp). The only deltas are the public C++ signatures
// (RB3 class-static vs DC3 namespace + `int` vs `unsigned int`).

#ifdef HX_NATIVE

#include "KeyChain.h"
#include <cstring>

static const unsigned char hiddenKeys[0x180] = {
    
};

static unsigned char asciiDigitToHex(char digit) {
    if (0x61 <= digit && digit <= 0x66)
        return digit - 0x61 + 0xa;
    else if (0x41 <= digit && digit <= 0x46)
        return digit - 0x41 + 0xa;
    else
        return digit - 0x30;
}

static void parseHex16(const char *input, unsigned char *output) {
    const char *src = input;
    for (unsigned int i = 0; i < 0x10; ++i) {
        unsigned int val = (asciiDigitToHex(src[0]) << 4) + asciiDigitToHex(src[1]);
        src += 2;
        *output++ = val;
    }
}

// Must use int (32-bit) — Xbox PPC long is 4 bytes, x86_64 long is 8.
// The LCG overflow behavior must match the original 32-bit arithmetic.
static int random(int l) {
    static int s_seed = 0xEB;
    if (l) s_seed = l;
    s_seed = s_seed * 0x19660E + 0x3C6EF35F;
    return s_seed;
}

static void swap(char &c1, char &c2) { char t = c1; c1 = c2; c2 = t; }
static int roll(int i) { return (i + 19) % 32; }

static void shuffle1(char *c) {
    for (int i = 0; i < 8; i++) {
        swap(c[roll(i * 4)], c[i * 4 + 2]);
        swap(c[roll(i * 4 + 3)], c[i * 4 + 1]);
    }
}
static void shuffle2(char *c) {
    for (int i = 0; i < 8; i++) {
        swap(c[(7 - i) * 4 + 1], c[i * 4 + 2]);
        swap(c[(7 - i) * 4], c[i * 4 + 3]);
    }
}
static void shuffle3(char *c) {
    for (int i = 0; i < 8; i++) {
        swap(c[roll((7 - i) * 4 + 1)], c[i * 4 + 2]);
        swap(c[(7 - i) * 4], c[i * 4 + 3]);
    }
}
static void shuffle4(char *c) {
    for (int i = 0; i < 8; i++) {
        swap(c[(7 - i) * 4 + 1], c[i * 4 + 2]);
        swap(c[roll((7 - i) * 4)], c[i * 4 + 3]);
    }
}
static void shuffle5(char *c) {
    for (int i = 0; i < 8; i++) {
        swap(c[(7 - i) * 4 + 1], c[roll(i * 4 + 2)]);
        swap(c[(7 - i) * 4], c[i * 4 + 3]);
    }
}
static void shuffle6(char *c) {
    for (int i = 0; i < 8; i++) {
        swap(c[(7 - i) * 4 + 1], c[i * 4 + 2]);
        swap(c[(7 - i) * 4], c[roll(i * 4 + 3)]);
    }
}
static void supershuffle(char *c) {
    shuffle1(c); shuffle2(c); shuffle3(c);
    shuffle4(c); shuffle5(c); shuffle6(c);
}

// Opaque predicate — on Xbox this was a volatile global at a fixed address.
// On native it's just a local counter to maintain the same execution pattern.
static volatile long sOpaqueCounter = 0;
static void opaquePredicate() { sOpaqueCounter++; }

static void memcpy_cs(unsigned char *dst, const unsigned char *src, unsigned int n) {
    for (unsigned int x = 0; x < n; x++) {
        opaquePredicate();
        dst[x] = src[x];
    }
}

static void mash(unsigned char *uc1, unsigned char *uc2) {
    unsigned int *ui1 = (unsigned int *)uc1;
    unsigned int *ui2 = (unsigned int *)uc2;
    for (int i = 0; i < 8; i++) ui1[i] ^= ui2[i];
}

static void revealKey(long *lp, unsigned char *uc) {
    for (int i = 0; i < 0xE; i++) supershuffle((char *)lp);
    mash((unsigned char *)lp, uc);
}

static void getKeyImpl(unsigned char *uc1, char *c, unsigned char *uc2) {
    revealKey((long *)uc1, uc2);
    memcpy_cs((unsigned char *)c, uc1, 0x20);
}

// -----------------------------------------------------------------------------
// Public RB3-shaped KeyChain interface (class with static methods, `int`
// width — see rb3/src/KeyChain.h).
// -----------------------------------------------------------------------------

void KeyChain::getMasher(unsigned char *uc) {
    // Xbox PPC (big-endian) byte-swaps masher values to little-endian.
    // On x86_64 (little-endian), values are already little-endian — no swap needed.
    // The original Xbox check was: byte[+3] == 1 -> true on big-endian -> swap to LE.
    unsigned int m = 1;
    int needs_byteswap = (*(((unsigned char *)&m) + 3) == 1); // big-endian = true
    unsigned int *masher_p = reinterpret_cast<unsigned int *>(uc);
    for (int i = 0; i < 8; i++) {
        *masher_p = random((0 == i) ? 0xEB : 0);
        if (needs_byteswap) {
            unsigned char *b = (unsigned char *)masher_p;
            unsigned char t0 = b[0], t1 = b[3];
            b[0] = t1; b[3] = t0;
            t0 = b[1]; t1 = b[2];
            b[1] = t1; b[2] = t0;
        }
        masher_p++;
    }
}

void KeyChain::getKey(int i, unsigned char *uc1, unsigned char *uc2) {
    unsigned char uchar_arr[32];
    char char_arr[32];
    memcpy_cs(uchar_arr, &hiddenKeys[i * 0x20], 0x20);
    getKeyImpl(uchar_arr, char_arr, uc2);
    parseHex16(char_arr, uc1);
}

int KeyChain::getNumKeys() { return 0xC; }

#endif // HX_NATIVE
