...
u32 EXIClearInterrupts(EXIChannel channel, BOOL exi, BOOL tc, BOOL ext) {
    u32 cpr;
    u32 prev;
...
BOOL EXIDma(EXIChannel channel, void *buffer, s32 length, u32 type, EXICallback cb) {
...
static void EXIIntrruptHandler(s32 interrupt, OSContext* context) {
    s32 chan;
    EXIControl* exi;
...
static void TCIntrruptHandler(s32 interrupt, OSContext* context) {
    s32 chan;
    EXIControl* exi;
    EXICallback callback;
    chan = (interrupt - 10) / 3;
...
static void EXTIntrruptHandler(s32 interrupt, OSContext* context) {
    s32 chan;
    EXIControl* exi;
    EXICallback callback;
    chan = (interrupt - 11) / 3;
