...
ISFSError nandCreateDir(const char *path, const u8 perm, const u8 attr,
  ...
  if (!privilege_flag && nandIsPrivatePath(absPath)) {
    ...
    if (async_flag) {
      return ISFS_CreateDirAsync(absPath, attr, owner, group,
                                 others, nandCallback, block);
    } else {
...
ISFSError nandMove(const char *path, const char *destDir,
  ...
  if (!privilege_flag &&
    ...
    if (async_flag) {
      return ISFS_RenameAsync(absOldPath, absNewPath,
                              nandCallback, block);
    } else {
  ...
  if (!privilege_flag && nandIsUnderPrivatePath(absPath)) {
    ...
    if (async_flag) {
      return ISFS_GetAttrAsync(
          absPath, &(stat->ownerId), &(stat->groupId),
      ...
      ISFSError result =
          ISFS_GetAttr(absPath, &stat->ownerId, &stat->groupId,
