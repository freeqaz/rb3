...
static void __wudOpenWiiFitCallback(NANDResult result, NANDCommandBlock *cmdBlock);
static void __wudOpenWiiFitFile(void);
static void __wudSeekWiiFitCallback(NANDResult result, NANDCommandBlock *cmdBlock);
static void __wudSeekWiiFitFile(void);
static void __wudUpdateWiiFitCallback(NANDResult result, NANDCommandBlock *cmdBlock);
static void __wudUpdateWiiFitFile(void);
static void __wudCloseWiiFitCallback(NANDResult result, NANDCommandBlock *cmdBlock);
...
static BOOL __wudStartSyncDevice(WUDSyncType syncType, s8 syncLopNum, u8 target, signed);
...
static void __wudSecurityEventStackCallback(tBTA_DM_SEC_EVT event, tBTA_DM_SEC *p_data);
static void
__wudSearchEventStackCallback(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data);
...
static void __wudPowerManageEventStackCallback(
    BD_ADDR p_bda, tBTM_PM_STATUS status, UINT16 value, UINT8 hci_status
);
...
static byte_t _wudSuperPeekPokeCmd[] = { 0x05, 0x00, 0x9a, 0x0e, 0x00,
                                         0x01, 0x00, 0x00, 0x00 };

static byte_t _wudPatchData[] = { 0x70,
                                  0x99,
                                  0x08,
                                  0x00, // address (little-endian)
                                  0xb4,
                                  0x00,
                                  0x00,
...
static byte_t _wudResetAuthCountCmd[] = { 0x30, 0x36, 0x08, 0x00, 0x00, 0x00, 0x00,
...
BOOL WUDIsLinkedWBC(void) { return _linkedWBC; }
...
static signed __wudSetPinCode(BD_ADDR dev_addr) {
    BD_ADDR_PTR myAddr =
        __rvl_wudcb.syncType == WUD_SYNC_TYPE_STANDARD ? __rvl_wudcb.hostAddr : dev_addr;
...
static BOOL __wudIsBusyScCmd(void) { return SCCheckStatus() == SC_STATUS_BUSY ? 1 : 0; }

static void __wudPrepareWBCDevInfo(BD_ADDR dev_addr, LINK_KEY link_key, void *param_3) {
    u8 offset = 0;
    ...
    if (link_key) {
        memcpy((void *)((u32)&_wudNandWbcInfo + offset), link_key, LINK_KEY_LEN);
    }
...
static WUDSyncState __wudSyncStoredLinkKeyToE2prom(void) {
    ...
    BTM_WriteStoredLinkKey(
        1, &p_info->devAddr, &p_info->linkKey, &__wudLinkKeyEventStackCallback
    );
...
static WUDSyncState __wudSyncStoredDevInfoToNand(void) {
    ...
    memset(&_scArray.devices, 0, sizeof _scArray.devices[i] * WUD_MAX_DEV_ENTRY_FOR_STD);
    ...
    while (infoList) {
        ...
        memcpy(
            &_scArray.devices[i].small,
            infoList->devInfo->small.devName,
...
static void __wudOpenWiiFitCallback(
    NANDResult result, NANDCommandBlock *cmdBlock __attribute__((unused))
) {
...
static void __wudSeekWiiFitCallback(
    NANDResult result, NANDCommandBlock *cmdBlock __attribute__((unused))
) {
    if (__rvl_wudcb.syncState != WUD_STATE_START) {
        __rvl_wudcb.syncState = result == RP_HEALTH_FILE_OFFSET ? WUD_STATE_WII_FIT_UPDATE
...
static void __wudUpdateWiiFitCallback(
    NANDResult result, NANDCommandBlock *cmdBlock __attribute__((unused))
) {
    if (__rvl_wudcb.syncState != WUD_STATE_START) {
...
static void __wudUpdateWiiFitFile(void) {
    ...
    NANDWriteAsync(
        &_wudNandFileInfo,
        &_wudNandWbcInfo,
        RP_HEALTH_FILE_WRITE_SIZE,
        &__wudUpdateWiiFitCallback,
        &_wudNandBlock
    );
}

static void __wudCloseWiiFitCallback(
    NANDResult result __attribute__((unused)),
    NANDCommandBlock *cmdBlock __attribute__((unused))
) {
    if (__rvl_wudcb.syncState != WUD_STATE_START)
...
static WUDSyncState __wudSyncComplete(void) {
    ...
    return __rvl_wudcb.syncType == WUD_SYNC_TYPE_STANDARD ? WUD_STATE_SYNC_DONE
...
static WUDSyncState __wudSyncDone(void) {
    ...
    WUDSyncDeviceCallback *syncCB =
        p_wcb->syncType == WUD_SYNC_TYPE_STANDARD ? p_wcb->syncStdCB : p_wcb->syncSmpCB;
    ...
    if (p_wcb->linkKeyState == WUD_STATE_START) {
        for (i = 0; i < WUD_MAX_DEV_ENTRY; i++) {
            ...
            if (p_info->at_0x5c == 1) {
                ...
                BTM_DeleteStoredLinkKey(p_info->devAddr, &__wudLinkKeyEventStackCallback);
                p_info->at_0x5c = 0;
...
static WUDInitState __wudInitDevInfo(void) {
    ...
    WUDDevInfo *p_info;
    for (i = 0, arrayNum = _scArray.num, a = 0; i < WUD_MAX_DEV_ENTRY_FOR_STD; i++) {
        if (!arrayNum)
            break;
            ...
            if (i < WUD_MAX_DEV_ENTRY_FOR_STD - 1) {
                for (j = i + 1; j < WUD_MAX_DEV_ENTRY_FOR_STD; j++) {
                    if (WUD_DEV_NAME_IS_CNT(_scArray.devices[j].small.devName)) {
                        ...
                        memset(&_scArray.devices[j], 0, sizeof _scArray.devices[i]);
        ...
        there:
            ...
            if (p_info) {
                ...
                memcpy(&p_info->small, &_scArray.devices[i].small, sizeof p_info->small);
    ...
    if (WUDIsLinkedWBC() && WUD_DEV_NAME_IS_WBC(_scArray.wbc.small.devName)) {
        ...
        // sizeof == 19?
        memcpy(_scArray.devices[a].small.devName, _scArray.wbc.small.devName, 19);
    ...
    for (i = 5, arrayNum = _spArray.num; i >= 0; i--) {
        ...
        if (p_info) {
            ...
            memcpy(&p_info->small, &_spArray.devices[i].small, sizeof p_info->small);
...
static void __wudNandResultCallback(
    NANDResult result, NANDCommandBlock *block __attribute__((unused))
) {
    // ?
#if !defined(NDEBUG)
    // clang-format off
...
static WUDInitState __wudGetDevInfoFromWiiFit(void) {
    ...
    if (!_wudNandBufPtr || !IS_ALIGNED(_wudNandBufPtr, 0x20)) {
        OSError_Line(
            2745,
            "Error: the workarea for synchronizing WBC pairing "
    ...
    case WUD_STATE_NAND_GET_DEV_INFO:
        ...
        memcpy(
            &wfDataCrc, _wudNandBufPtr + RP_HEALTH_FILE_OFFSET + 0x80, sizeof wfDataCrc
        );

        _wudNandWbcCrc = __wudCalcWiiFitCrc1(_wudNandBufPtr, RP_HEALTH_FILE_OFFSET);
        wfCalcCrc = __wudCalcWiiFitCrc2(
            _wudNandBufPtr + RP_HEALTH_FILE_OFFSET, 128, _wudNandWbcCrc
        );
        ...
        memcpy(
            &_scArray.wbc.small,
            (void *)((u32)&_wudNandWbcInfo + 22),
            sizeof _scArray.wbc.small
        );
        memcpy(
            (void *)((u32)&_scArray.wbc + 26),
            &_wudNandWbcInfo.scDevInfo.small.devName,
            16
        );
        SCSetBtDeviceInfoArray(&_scArray);
    ...
    case WUD_STATE_NAND_CLOSE:
        ...
        result =
            NANDCloseAsync(&_wudNandFileInfo, &__wudNandResultCallback, &_wudNandBlock);
...
BOOL WUDInit(void) {
    ...
    OSAssertMessage_Line(
        3086,
        p_wcb->allocFunc && p_wcb->freeFunc,
    ...
    OSSetPeriodicAlarm(
        &p_wcb->alarm, OSGetTime(), OSMillisecondsToTicks(10), &__wudInitHandler0
    );
...
void WUDShutdown(BOOL saveSimpleDevs) {
    ...
    memset(_scArray.devices, 0, sizeof _scArray.devices[0] * WUD_MAX_DEV_ENTRY_FOR_STD);
    ...
    if (saveSimpleDevs) {
        ...
        while (infoList) {
            ...
            memcpy(
                &_spArray.devices[i].small,
                &infoList->devInfo->small,
                sizeof _spArray.devices[i].small
            );

            memcpy(
                &_spArray.devices[i].linkKey, &infoList->devInfo->linkKey, LINK_KEY_LEN
            );
    ...
    OSCreateAlarm(&p_wcb->alarm);
    OSSetPeriodicAlarm(
        &p_wcb->alarm, OSGetTime(), OSMillisecondsToTicks(10), &__wudShutdownHandler0
    );
...
static BOOL
__wudStartSyncDevice(WUDSyncType syncType, s8 syncLoopNum, u8 target, signed param_4) {
    wud_cb_st *p_wcb = &__rvl_wudcb;
    ...
    if (libStatus == WUD_LIB_STATUS_3 && !WUDIsBusy()) {
        ...
        OSCreateAlarm(&p_wcb->alarm);
        OSSetPeriodicAlarm(
            &p_wcb->alarm, OSGetTime(), OSMillisecondsToTicks(20), &__wudSyncHandler0
        );
...
BOOL WUDStartSyncDevice(void) { return __wudStartSyncStandard(0); }

BOOL WUDStartFastSyncSimple(void) { return __wudStartSyncSimple(1); }
...
BOOL WUDStopSyncSimple(void) { return __wudStopSyncDevice(); }
...
static void __wudModuleReboot(void) { BTM_DeviceReset(&__wudModuleRebootCallback); }
...
static void __wudInstallPatch(void) {
    ...
    memcpy(
        &buf[offsetof(struct wud_patch_install_cmd_array, installCmds)],
...
static void __wudWritePatch(void) {
    ...
    for (i = 0; i < (s32)length; i++) {
        buf[(int)sizeof(u32) + i] = _wudPatchData
            [sizeof(_wudPatchSize) + sizeof(_wudPatchOffset) + _wudPatchOffset + i];
    }

    _wudPatchOffset += length;
    BTM_VendorSpecificCommand(
        BT_VSC_NINTENDO_WRITE_PATCH,
        (int)sizeof(u32) + length,
        buf,
...
static void __wudRemovePatch(void) {
    BTM_VendorSpecificCommand(
        BT_VSC_NINTENDO_INSTALL_PATCH,
        sizeof _wudPatchRemoveCmd,
        &_wudPatchRemoveCmd,
        &__wudRemovePatchCallback
    );
}

static void __wudSuperPeekPokeCallback(tBTM_VSC_CMPL *p1 __attribute__((unused))) {
    __wudRemovePatch();
}

static void __wudSuperPeekPoke(void) {
    BTM_VendorSpecificCommand(
        BT_VSC_NINTENDO_SUPER_PEEK_POKE,
        sizeof _wudSuperPeekPokeCmd,
...
static void __wudInitSub(void) {
    ...
    BTM_PmRegister(
        BTM_PM_REG_SET | BTM_PM_REG_NOTIF,
        &p_wcb->pmID,
...
static void __wudEnableStack(void) {
    ...
    OSCreateAlarm(&p_wcb->alarm);
    OSSetPeriodicAlarm(
        &p_wcb->alarm, OSGetTime(), OSMillisecondsToTicks(10), &__wudStackHandler0
    );
}
...
void WUDiRemoveDevice(BD_ADDR dev_addr) {
    ...
    tBTM_STATUS status __attribute__((unused)) = BTA_DmRemoveDevice(p_info->devAddr);
...
void WUDiMoveTopSmpDevInfoPtr(WUDDevInfo *dev_info) {
    ...
    for (i = 0; i < WUD_MAX_DEV_ENTRY_FOR_SMP; i++) {
        ...
        if (WUD_BDCMP(
                p_wcb->smpListHead->devInfo->devAddr, p_wcb->smpList[i].devInfo->devAddr
            )
            == 0) {
            break;
        }
        ...
        if (WUD_BDCMP(
                p_wcb->smpListTail->devInfo->devAddr, p_wcb->smpList[i].devInfo->devAddr
            )
            == 0) {
...
void WUDiMoveTopOfDisconnectedSmpDevice(WUDDevInfo *dev_info) {
    ...
    for (i = 0; i < WUD_MAX_DEV_ENTRY_FOR_SMP; i++) {
        ...
        for (p_list = p_wcb->smpListHead; p_list; p_list = p_list->next) {
            ...
            if (WUD_BDCMP(
                    p_wcb->smpListHead->devInfo->devAddr,
...
void WUDiMoveTopStdDevInfoPtr(WUDDevInfo *dev_info) {
    ...
    for (i = 0; i < WUD_MAX_DEV_ENTRY_FOR_STD; i++) {
        ...
        if (WUD_BDCMP(
                p_wcb->stdListHead->devInfo->devAddr, p_wcb->stdList[i].devInfo->devAddr
            )
            == 0) {
            break;
        }
        ...
        if (WUD_BDCMP(
                p_wcb->stdListTail->devInfo->devAddr, p_wcb->stdList[i].devInfo->devAddr
            )
            == 0) {
...
void WUDiMoveTopOfDisconnectedStdDevice(WUDDevInfo *devInfo) {
    ...
    for (i = 0; i < WUD_MAX_DEV_ENTRY_FOR_STD; i++) {
        ...
        for (p_list = p_wcb->stdListHead; p_list; p_list = p_list->next) {
            ...
            if (WUD_BDCMP(
                    p_wcb->stdListHead->devInfo->devAddr,
...
BOOL WUDIsBusy(void) {
    ...
    if (p_wcb->syncState == WUD_STATE_START && p_wcb->deleteState == WUD_STATE_START
...
static void __wudSecurityEventStackCallback(tBTA_DM_SEC_EVT event, tBTA_DM_SEC *p_data) {
    ...
    case BTA_DM_AUTHORIZE_EVT: {
        tBTA_DM_AUTHORIZE *dmAuthorize __attribute__((unused)) = &p_data->authorize;
    } break;
        ...
        if (p_info) {
            ...
            if (dmLinkDown->status == HCI_ERR_PEER_POWER_OFF) {
                for (i = 0; i < WUD_MAX_CHANNELS; i++) {
                    if (WUD_BDCMP(_scArray.devices[10 + i].devAddr, dmLinkDown->bd_addr)
                        == 0) {
                        memset(
                            &_scArray.devices[10 + i], 0, sizeof _scArray.devices[10 + i]
                        );
                        _scFlush = TRUE;
                    }
                }
    ...
    case BTA_DM_BUSY_LEVEL_EVT: {
        tBTA_DM_BUSY_LEVEL *dmBusyLevel __attribute__((unused)) = &p_data->busy_level;
    } break;
    }
}
...
__wudSearchEventStackCallback(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH *p_data) {
    ...
    case BTA_DM_DISC_RES_EVT: {
        ...
        memcpy(_wudDiscResp.at_0x006, dmDiscRes->bd_name, sizeof _wudDiscResp.at_0x006);
...
static void
__wudVendorSpecificEventStackCallback(UINT8 len __attribute__((unused)), UINT8 *p) {
    (void)len; // mhm
...
static void __wudPowerManageEventStackCallback(
    ...
    UINT16 value __attribute__((unused)),
...
void WUDSetDeviceHistory(WUDChannel chan, BD_ADDR dev_addr) {
    if (!dev_addr) {
        memset(&_scArray.devices[10 + chan], 0, sizeof _scArray.devices[10 + chan]);
    } else {
...
void WUDiShowFatalErrorMessage(void) { return; }
