...
WiiProfile::WiiProfile()
    : mSlot(-1), mId(0), mFlags(0), mHasSeenFirstTimeInstrumentFlags(0) {}
...
int WiiProfile::SaveSize() { REPORT_SIZE("WiiProfile", 59); }
...
const char *WiiProfile::GetName() const { return mProfileName; }
...
inline bool WiiProfile::IsFlag(uint Flag) const { return mFlags & Flag; }
#pragma pop
...
WiiProfileMgr::WiiProfileMgr() : mHasLoaded(0), mDirty(0) { mSaveSizeMethod = &SaveSize; }

WiiProfileMgr::~WiiProfileMgr() {}

void WiiProfileMgr::Init(int rev, int revWii) { Clear(rev, revWii); }
...
void WiiProfileMgr::LoadFixed(FixedSizeSaveableStream &fs, int) {
    ...
    if (rev != sSaveVersion || revWii != sSaveVersionWii) {
        MILO_WARN(
            "**WiiProfileMgr::Tried to load a bad or out-of-date version (%d | %d) vs (%d | %d)!\n",
            rev,
            revWii,
            sSaveVersion,
            sSaveVersionWii
...
bool WiiProfileMgr::DeleteProfile(int iii) {
    if (iii <= 3U && IsIndexValid(iii)) {
        if (IsIndexRegistered(iii)) {
            if (!AddIdToDeleteQueue(GetIdForIndex(iii)))
                return false;
        }
...
int WiiProfileMgr::GetIndexForPad(int pad) const {
    if (pad <= 3U && mPadProfileIndex[pad] >= 0)
        return mPadProfileIndex[pad];
    else
        return -1;
}
...
const char *WiiProfileMgr::GetNameForIndex(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].mProfileName;
    else
        return nullptr;
}
...
bool WiiProfileMgr::IsIndexValid(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].IsFlag(1);
    else
        return false;
}
...
bool WiiProfileMgr::IsIndexSaved(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].IsFlag(4);
    else
        return false;
}
...
bool WiiProfileMgr::IsIndexRegistered(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].IsFlag(2);
    else
        return false;
}
...
bool WiiProfileMgr::IsIndexLoaded(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].IsFlag(0x4000);
    else
        return false;
}
...
void WiiProfileMgr::SetIndexLoaded(int idx, bool loaded) {
    if (idx <= 3U)
        mWiiProfiles[idx].SetFlag(0x4000, loaded);
}

bool WiiProfileMgr::IsIndexSwapping(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].IsFlag(0x2000);
    else
        return false;
}
...
bool WiiProfileMgr::IsIndexLocked(int idx) const {
    if (idx <= 3U)
        return mWiiProfiles[idx].IsFlag(0x8000);
    else
        return false;
}
...
void WiiProfileMgr::SetIndexLocked(int idx, bool locked) {
    if (idx <= 3U)
        mWiiProfiles[idx].SetFlag(0x8000, locked);
}
...
int WiiProfileMgr::GetHasSeenFirstTimeInstrumentFlagsForUser(const LocalUser *user
) const {
    int idx = GetIndexForUser(user);
    if (idx >= 0)
        return mWiiProfiles[idx].mHasSeenFirstTimeInstrumentFlags;
    else
        return 0;
}

void WiiProfileMgr::SetHasSeenFirstTimeInstrumentFlagsForUser(
    const LocalUser *user, int flags, bool on
) {
    int idx = GetIndexForUser(user);
    if (idx >= 0) {
        if (on)
            mWiiProfiles[idx].mHasSeenFirstTimeInstrumentFlags |= flags;
        else
            mWiiProfiles[idx].mHasSeenFirstTimeInstrumentFlags &= ~flags;
    }
}