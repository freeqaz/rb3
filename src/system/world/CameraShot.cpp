...
CamShot::CamShot()
    : mKeyframes(this), mLoopKeyframe(0), mNear(1.0f), mFar(1000.0f), mFilter(0.9f),
      ...
      mFlags(0), mLooping(0), mUseDepthOfField(1), mPS3PerPixel(1), unk120p4(1),
      mShotOver(0), mHidden(0), unk120p1(0) {}
...
bool CamShot::PlatformOk() const {
    if (LOADMGR_EDITMODE || mPlatformOnly == kPlatformNone
...
BEGIN_LOADS(CamShot)
    ...
    if (gRev > 0x22)
        ...
        bs >> state;
        if (state == 1)
            mPlatformOnly = kPlatformXBox;
        else if (state == 2)
            mPlatformOnly = kPlatformPS3;
        else
            mPlatformOnly = kPlatformNone;
    }
    if (gRev < 1)
        LOAD_SUPERCLASS(RndAnimatable)
...
CamShotFrame::CamShotFrame(Hmx::Object *o, const CamShotFrame &frame)
    : mDuration(frame.mDuration), mBlend(frame.mBlend), mBlendEase(frame.mBlendEase),
      mWorldOffset(frame.mWorldOffset), mScreenOffset(frame.mScreenOffset),
      mShakeNoiseAmp(frame.mShakeNoiseAmp), mShakeNoiseFreq(frame.mShakeNoiseFreq),
      mFocusBlurMultiplier(frame.mFocusBlurMultiplier), mTargets(frame.mTargets),
      mCamShot(dynamic_cast<CamShot *>(o)), mParent(frame.mParent),
      mFocusTarget(frame.mFocusTarget), mFOV(frame.mFOV), mZoomFOV(frame.mZoomFOV),
      mBlurDepth(frame.mBlurDepth), mMaxBlur(frame.mMaxBlur), mMinBlur(frame.mMinBlur),
      mBlendEaseMode(frame.mBlendEaseMode), mUseParentNotation(frame.mUseParentNotation),
      mParentFirstFrame(0) {
...
void CamShotFrame::Load(BinStream &bs) {
    ...
    if (CamShot::gRev > 0x1A) {
        if (CamShot::gRev > 0x2B)
            ...
            mFocusTarget = LoadSubPart(bs, mCamShot);
    }
    if (CamShot::gRev > 0x2B)
        bs >> mParent;
    else
        mParent = LoadSubPart(bs, mCamShot);
    bool b1;
    bs >> b1;
...
void CamShotFrame::Interp(const CamShotFrame &frame, float f1, float f2, RndCam *cam) {
    ...
    if (b10) {
        ...
        if (towardFocus) {
            ...
            if (thasTarget)
                d9 = Distance(frame.mLastTargetPos, tf130.v);
        }
        if (!focus && !hasTarget) {
...
inline float ComputeFOVScale(float fov) { return 24.0f / (std::tan(fov / 2.0f) * 2.0f); }

inline float ScaleToFOV(float scale) { return std::atan(24.0f / (scale * 2.0f)) * 2.0f; }

Symbol FOV_to_LensSym(float fov) {
    float scaled = ComputeFOVScale(fov);
    if (ApproxEq(scaled, 15.0f))
        return "15mm";
    else if (ApproxEq(scaled, 20.0f))
        return "20mm";
    else if (ApproxEq(scaled, 24.0f))
        return "24mm";
    else if (ApproxEq(scaled, 28.0f))
        return "28mm";
    else if (ApproxEq(scaled, 35.0f))
        return "35mm";
    else if (ApproxEq(scaled, 50.0f))
        return "50mm";
    else if (ApproxEq(scaled, 85.0f))
        return "85mm";
    else if (ApproxEq(scaled, 135.0f))
        return "135mm";
    else if (ApproxEq(scaled, 200.0f))
        return "200mm";
    else
        return "Custom";
}
...
BEGIN_CUSTOM_PROPSYNC(CamShotFrame)
    ...
    SYNC_PROP_SET(
        use_parent_rotation,
        o.mUseParentNotation != 0,
        o.mUseParentNotation = _val.Int() != 0
    )
    SYNC_PROP_SET(
        parent_first_frame,
        o.mParentFirstFrame != 0,
        o.mParentFirstFrame = _val.Int() != 0
    )
    SYNC_PROP_SET(
        field_of_view, o.FieldOfView() * RAD2DEG, o.SetFieldOfView(_val.Float() * DEG2RAD)
    )
    SYNC_PROP_SET(
        lens_mm,
        ComputeFOVScale(o.FieldOfView()),
        o.SetFieldOfView(ScaleToFOV(_val.Float()))
    )
    SYNC_PROP_SET(lens_preset, FOV_to_LensSym(o.FieldOfView()), {
        float fov = LensSym_to_FOV(_val.Sym());
        if (fov != -1.0f)
            o.SetFieldOfView(fov);
        else
            o.SetFieldOfView(o.FieldOfView() + 0.00010011921f);
    })
    ...
    SYNC_PROP_SET(
        zoom_fov,
        o.ZoomFieldOfView() * RAD2DEG,
        o.SetZoomFieldOfView(_val.Float() * DEG2RAD)
    )
END_CUSTOM_PROPSYNC
