...
void RndFont::UpdateChars() {
    if (IsPacked()) {
        ...
        if (t)
            SetBitmapSize(mCellSize, t->Width(), t->Height());
    } else {
    ...
    if (bmap) {
        ...
        for (int i = 0; i < mChars.size(); i++) {
            ...
            if (i5 != 0 || (!haswrap && ia0 <= 0)) {
                MaxEq(ia0, 0);
                unsigned char row =
                    bmap->ColumnNonTransparent(ia0, i2, i2 + (int)mCellSize.y, &iptr);
                if (row) {
                    errStr += MakeString(
        ...
        if (errStr.length() != 0) {
            ...
            MILO_WARN("No bleeding over found.  \n");
    }
}

void RndFont::GetKerning(std::vector<KernInfo> &info) const {
    if (mKerningTable)
        mKerningTable->GetKerning(info);
    else
        info.clear();
    return;
    if (!mKerningTable)
...
String RndFont::GetASCIIChars() { return WideVectorToASCII(mChars); }
