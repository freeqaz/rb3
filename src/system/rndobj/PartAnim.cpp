...
RndParticleSysAnim::RndParticleSysAnim()
    : mParticleSys(this, 0), mKeysOwner(this, this) {}
...
void RndParticleSysAnim::Replace(Hmx::Object *from, Hmx::Object *to) {
    ...
    if (mKeysOwner == from) {
        if (!to)
            ...
            mKeysOwner = dynamic_cast<RndParticleSysAnim *>(to)->mKeysOwner;
    }
}
...
BEGIN_LOADS(RndParticleSysAnim)
    ...
    if (rev < 2) {
        ...
        for (Keys<float, float>::iterator kIt = fKeys.begin(); kIt != fKeys.end();
             ++kIt) {
            Key<Vector2> toPush;
