...
RndTransAnim::RndTransAnim()
    : mTrans(this), mTransSpline(0), mScaleSpline(0), mRotSlerp(0), mRotSpline(0),
      mRotKeys(), mTransKeys(), mScaleKeys(), mKeysOwner(this, this), mRepeatTrans(0),
      mFollowPath(0) {}
...
void RndTransAnim::Replace(Hmx::Object *from, Hmx::Object *to) {
    ...
    if (mKeysOwner == from) {
        if (!to)
            ...
            mKeysOwner = dynamic_cast<RndTransAnim *>(to)->mKeysOwner;
    }
}
...
void RndTransAnim::Load(BinStream &bs) {
    ...
    if (rev < 3) {
        ...
        if (rev == 2 || numKeys != 0) {
            ...
            for (Keys<Vector3, Vector3>::iterator it = mTransKeys.begin();
                 it != mTransKeys.end();
                 ++it) {
                int i1, i2, i3;
                Vector3 v1, v2;
        ...
        if (rev == 2 || numKeys != 0) {
            ...
            for (Keys<Hmx::Quat, Hmx::Quat>::iterator it = mRotKeys.begin();
                 it != mRotKeys.end();
                 ++it) {
                int i1, i2, i3;
                Hmx::Quat v1, v2;
    ...
    if (rev > 3) {
        ...
        if (rev < 3) {
            ...
            if (rev == 2 || numKeys != 0) {
                ...
                for (Keys<Vector3, Vector3>::iterator it = mScaleKeys.begin();
                     it != mScaleKeys.end();
                     ++it) {
                    int i1, i2, i3;
                    Vector3 v1, v2;
