...
void RndMeshAnim::Replace(Hmx::Object *from, Hmx::Object *to) {
    ...
    if (mKeysOwner == from) {
        if (!to)
            ...
            mKeysOwner = dynamic_cast<RndMeshAnim *>(to)->mKeysOwner;
    }
}
    ...
    if (mMesh) {
        if ((mMesh->GetMutable() & 0x1F) == 0) {
            ...
            if (!VertColorsKeys().empty()) {
                ...
                InterpVertData<Hmx::Color32, GetVertColor>(
                    prev->value, next->value, ref, mMesh->Verts(), blend
                );
                syncnum |= 0x1F;
            }
...
void RndMeshAnim::ShrinkVerts(int num) {
    for (Keys<std::vector<Vector3>, std::vector<Vector3> >::iterator it =
         ...
         it != VertPointsKeys().end();
         ++it) {
        it->value.resize(num);
    }
    for (Keys<std::vector<Vector3>, std::vector<Vector3> >::iterator it =
             VertNormalsKeys().begin();
         it != VertNormalsKeys().end();
         ++it) {
        it->value.resize(num);
    }
    for (Keys<std::vector<Vector2>, std::vector<Vector2> >::iterator it =
             VertTexsKeys().begin();
         it != VertTexsKeys().end();
         ++it) {
        it->value.resize(num);
    }
    for (Keys<std::vector<Hmx::Color32>, std::vector<Hmx::Color32> >::iterator it =
             VertColorsKeys().begin();
         it != VertColorsKeys().end();
         ++it) {
        it->value.resize(num);
    }
}
