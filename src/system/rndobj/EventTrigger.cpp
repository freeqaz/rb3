...
BinStream &operator>>(BinStream &bs, EventTrigger::ProxyCall &pcall) {
    bs >> pcall.mProxy;
    bs >> pcall.mCall;
    if (EventTrigger::gRev > 10)
        pcall.mEvent.Load(bs, true, pcall.mProxy);
    return bs;
}
    ...
    if (gRev < 5) {
        ...
        while (count-- != 0) {
            ...
            if (count != 0) {
                ...
                curTrig = mNextLink;
                curTrig->SetName(
                    MakeString("%s_%d.trig", FileGetBase(Name(), 0), count), dir
                );
            }
        }
    }
...
BEGIN_LOADS(EventTrigger)
    ...
    if (gRev > 0xD)
        bs >> mResetTriggers;
    if (gRev > 0xE)
        LOAD_BITFIELD(bool, mEnabledAtStart)
    if (gRev > 0xF) {
        int i;
        bs >> i;
        mAnimTrigger = i;
...
void EventTrigger::Replace(Hmx::Object *from, Hmx::Object *to) {
    ...
    for (ObjVector<ProxyCall>::iterator it = mProxyCalls.begin();
         it != mProxyCalls.end();) {
...
void EventTrigger::ConvertParticleTriggerType() {
    if (!unkdf && Type() == "particle_trigger") {
        ...
        for (int i = 0; i < systemsArr->Size(); i++) {
            ...
            if (launcher)
                mPartLaunchers.push_back(launcher);
        }
        SetTypeDef(nullptr);
    }
    unkdf = true;
...
BEGIN_PROPSYNCS(EventTrigger)
    ...
    SYNC_PROP_SET(
        enabled_at_start, mEnabledAtStart, mEnabled = mEnabledAtStart = _val.Int()
    ) {
