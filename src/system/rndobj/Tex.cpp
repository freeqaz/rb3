...
void RndTex::SetPowerOf2() { mIsPowerOf2 = PowerOf2(mWidth) && PowerOf2(mHeight); }
...
void RndTex::PreLoad(BinStream &bs) {
    ...
    if (gRev > 9 && !bs.Cached()) {
        mLoader = new FileLoader(
            mFilepath, CacheResource(mFilepath.c_str(), this), kLoadFront, 0, false, true, 0
        );
    } else {
...
void RndTex::PostLoad(BinStream &bs) {
    ...
    if (mFilepath.empty()) {
        if (strcmp(Name(), "movie.tex") != 0 && strcmp(Name(), "movie_splash.tex") != 0
            && (mType & kRendered)) {
            while (mWidth > 0x100)
                mWidth /= 2;
            while (mHeight > 0x100)
...
BEGIN_COPYS(RndTex)
    ...
    BEGIN_COPYING_MEMBERS
        ...
        if (ty == kCopyFromMax && mType != c->mType)
            return;
        PresyncBitmap();
        COPY_MEMBER(mType)
        COPY_MEMBER(mWidth)
...
void RndTex::SaveBitmap(const char *) { MILO_ASSERT(0, 1033); }
