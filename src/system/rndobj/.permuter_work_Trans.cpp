...
template <class _T>
__declspec(noinline) auto _outline_WorldXfm(_T* _obj) -> decltype(_obj->WorldXfm()) {
    return _obj->WorldXfm();
}

void RndTransformable::ApplyDynamicConstraint() {
    if (mConstraint == kTargetWorld) {
        mWorldXfm = _outline_WorldXfm(mTarget);
    } else if (mConstraint == kShadowTarget) {
        Plane pl50;
        Transform tf40;
        Transpose(_outline_WorldXfm(mTarget), tf40);
        ...
        Multiply(mWorldXfm, tf40, mWorldXfm);
        Multiply(mWorldXfm, _outline_WorldXfm(mTarget), mWorldXfm);
    } else if (RndCam::sCurrent) {
        Vector3 v60;
        const Transform &currentWorld = _outline_WorldXfm(RndCam::sCurrent);
        if (mPreserveScale) {
        ...
        case kBillboardXZ:
            ...
            break;
        case kLookAtTarget:
            Subtract(_outline_WorldXfm(mTarget).v, mWorldXfm.v, mWorldXfm.m.y);
            Normalize(mWorldXfm.m, mWorldXfm.m);
            break;
        }
        if (mPreserveScale)
