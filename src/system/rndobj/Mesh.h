...
public:
    ...
    public:
        Vert()
            : pos(0, 0, 0), norm(0, 1, 0), boneWeights(0, 0, 0, 0), color(-1), uv(0, 0) {
            for (int i = 0; i < 4; i++)
                boneIndices[i] = 0;
        }
    ...
    /** Copy the bones from the supplied Mesh. If the other Mesh is NULL, clear our
     * current bone list.
    ...
    /** Routine to load any relevant vertex data from the BinStream before the main Load
     * method executes. */
    void PreLoadVertices(BinStream &);
    /** Routine to load any relevant vertex data from the BinStream after the main Load
     * method executes. */
     ...
     * @param [out] idx The index of the first invalid bone, or the bone vector's size if
     * everything is valid.
...
public:
    ...
    virtual const std::vector<SyncMeshCB::Vert> &GetVerts(RndMesh *) const = 0; // fix
                                                                                // return
                                                                                // type
};
