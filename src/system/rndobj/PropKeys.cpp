...
void SetPropKeysRev(int rev) { PropKeys::gRev = rev; }
...
PropKeys::PropKeys(Hmx::Object *targetOwner, Hmx::Object *target)
    : mTarget(targetOwner, target), mProp(0), mTrans(0), mInterpHandler(),
      mLastKeyFrameIndex(-2), mKeysType(kFloat), mInterpolation(kLinear),
      mPropExceptionID(kNoException), unk18lastbit(0) {}
...
void PropKeys::SetProp(DataNode &node) {
    if (node.Type() == kDataArray) {
        ...
        MILO_WARN("unknown prop set type");
    SetPropExceptionID();
}
...
void PropKeys::SetTarget(Hmx::Object *o) {
    if (mTarget.Ptr() != o) {
        bool b2 = true;
        bool b1 = false;
        if (mProp && GetPropertyVal(o, mProp, false) != 0)
            b1 = true;
        if (!b1
            && !(
...
void PropKeys::ReSort() {
    switch (mKeysType) {
    case kFloat:
        // mystery vector method - fn_806280C0 in retail, scratch:
        // https://decomp.me/scratch/5Vpiu
        AsFloatKeys().Sort();
        break;
    case kColor:
    ...
    case kObject:
        ...
        break;
    case kBool:
        // mystery vector method - fn_80627E24 in retail, scratch:
        // https://decomp.me/scratch/7TOeo
        AsBoolKeys().Sort();
        break;
    case kSymbol:
...
void Interp(
    const ObjectStage &stage1, const ObjectStage &stage2, float f, Hmx::Object *&obj
) {
...
void PropKeys::Load(BinStream &bs) {
    if (gRev < 7)
        ...
        if (gRev >= 8)
            ...
            iVal = (mKeysType == kObject || mKeysType == kBool) == 0;
...
inline bool CheckVectorDiff(const Vector3 &v1, const Vector3 &v2, float f) {
    return std::fabs(v1.x - v2.x) < f && std::fabs(v1.y - v2.y) < f
        && std::fabs(v1.z - v2.z) < f;
}
