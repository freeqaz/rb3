...
DECOMP_FORCEACTIVE(
    Mesh,
    "%s: exceeds bone limit of %d",
    "BSP tree outside bounding box",
...
RndMesh::RndMesh()
    : mMat(this), mGeomOwner(this, this), mBones(this), mMutable(0),
      mVolume(kVolumeTriangles), mBSPTree(0), mMultiMesh(0), mCompressedVerts(0),
...
void RndMesh::PostLoadVertices(BinStream &bs) {
    ...
    if (b58) {
        ...
        b4 = false;
        if (loadedCompressedSize == 0 && loadedVersion == 0)
            b4 = true;
        if (!b4) {
            MILO_WARN(
...
bool RndMesh::CacheStrips(BinStream &bs) {
    bool ret = false;
    if (bs.Cached() && bs.GetPlatform() == kPlatformWii && mGeomOwner == this
        && !mFaces.empty() && !mVerts.empty() && !(mMutable & 0x20))
        ret = true;
    return ret;
}
...
void RndMesh::CreateStrip(
    int i, int j, Striper &striper, STRIPERRESULT &sr, bool onesided
) {
    STRIPERCREATE sc;
...
void RndMesh::PostLoad(BinStream &bs) {
    ...
    if (gAltRev < 3 && NumBones() > 1) {
        MILO_WARN(
            "--->Arvin/Diana: Skinned mesh needs to be re-exported: %s", PathName(this)
        );
    }
}
#pragma pop
...
void RndMesh::SetZeroWeightBones() {
    if (mBones.size() >= 2) {
        for (int i = 0; i < mVerts.size(); i++) {
            ...
            if (curvert.boneWeights.GetY() == 0)
                curvert.boneIndices[1] = curvert.boneIndices[0];
            if (curvert.boneWeights.GetZ() == 0)
                curvert.boneIndices[2] = curvert.boneIndices[0];
            if (curvert.boneWeights.GetW() == 0)
...
DataNode RndMesh::OnConfigureMesh(const DataArray *da) {
    if (Type() != configurable_mesh)
        MILO_WARN("Can't configure nonconfigurable mesh %s\n", Name());
    else {
