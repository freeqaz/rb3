...
const char *RndText::FindPathName() {
    if (Name() && !*Name() && !Dir() && TransParent()) {
        return MakeString(
            "%s inside %s", ClassName().Str(), TransParent()->FindPathName()
        );
    } else
...
void RndText::SetData(
    ...
    if (mAlign != a || mCapsMode != caps || mFont != font || mLeading != leading
        || mWrapWidth != wrapwidth || mTextMarkup != markup || mStyle.size != size
        || mStyle.italics != italics || mStyle.color != col
        || mFixedLength != fixedLength) {
        SetFont(font);
        mAlign = a;
        mCapsMode = caps;
...
void RndText::SetAltStyle(
    RndFont *font, float size, const Hmx::Color32 *col, float z, float italics, bool b
) {
...
void RndText::SetAltSizeAndZOffset(float f1, float f2) {
    if (mAltStyle.size == f1 && mAltStyle.zOffset == f2)
        return;
...
float RndText::GetStringWidthUTF8(
    const char *cc1, const char *cc2, bool bbb, RndText::Style *style
) const {
                ...
                if (definingFont) {
                    if (font == definingFont) {
                        SetupCharVerts(
                            ...
                            curLine.unk28.v.y,
                            f3 + style.zOffset,
                            style.size * definingFont->CellDiff(),
                            f4,
                            style,
                            definingFont,
                            i14,
    ...
    if (!mMeshMap.empty()) {
        ...
        while (*theStrstr != '\0') {
            while ((*theStrstr == '<' && mTextMarkup)) {
                theStrstr =
                    ParseMarkup(theStrstr, &localStyle, style.size, style.zOffset);
            }
            if (*theStrstr != '\0') {
                unsigned short use6;
        ...
        FOREACH (it, mMeshMap) {
            ...
            while (*curStrStr != '\0') {
                while (*curStrStr == '<' && mTextMarkup) {
                    curStrStr =
                        ParseMarkup(curStrStr, &mapStyle, style.size, style.zOffset);
                }
                if (*curStrStr != '\0') {
...
void RndText::GetStringDimensions(
    float &f1, float &f2, std::vector<Line> &lines, const char *cc, float size
) {
    lines.clear();
...
BEGIN_PROPSYNCS(RndText)
    ...
    SYNC_PROP_SET(color, mStyle.color.Opaque(), Hmx::Color32 col(_val.Int());
                  col.a = mStyle.color.a;
                  SetColor(col))
    SYNC_PROP_SET(alpha, mStyle.color.fa(), Hmx::Color32 col(mStyle.color);
                  col.SetAlpha(_val.Float());
                  SetColor(col))
