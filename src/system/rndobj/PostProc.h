...
public:
    ...
    static void Reset();
#ifdef HX_NATIVE
    // Read-only accessors used by the native post-process pass
    // (milo-native-engine src/gfx/PostProcPass.cpp) to build its uniform block.
    ...
    /** "Attenuation (scattering amount) of light streak.\n0.9 to 0.95 is the sweet
     * spot.". Ranges from -2 to 2. */
    ...
    /** "Whether or not to use the velocity motion blur effect. Should be enabled almost
     * all the time." */
    bool mMotionBlurVelocity; // 0x184
    /** "Gradient map settings" */
    /** "Gradient map; this texture should be layed out horizontally such that the color
    ...
    /** "This scales the refraction texture before distorting the screen, in the X and Y
     * directions." */
    Vector2 mRefractScale; // 0x1b4
    /** "The amount to offset the refraction texture, in the X and Y directions. This is a
     * fixed amount to offset the refraction effect." */
    Vector2 mRefractPanning; // 0x1bc
    /** "The velocity to scroll the refraction texture, in the X and Y directions. The
     * value is specified in units per second, and will offset the refraction effect over
     * time." */
     ...
     * artifact where color channels are slightly shifted. This is useful to simulate old
     * cameras, poor quality video, or underwater scenes." */
