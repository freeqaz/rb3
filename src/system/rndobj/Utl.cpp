...
bool GroupedUnder(RndGroup *grp, Hmx::Object *o) {
    for (ObjPtrList<Hmx::Object>::iterator pit = grp->mObjects.begin();
         pit != grp->mObjects.end();
         ++pit) {
        if (*pit == o)
            return true;
...
void CalcSphere(RndTransAnim *a, Sphere &s) {
    ...
    if (!a->TransKeys().empty()) {
        ...
        Box box;
        Vector3 vec;
        for (Keys<Vector3, Vector3>::iterator it = a->TransKeys().begin();
             it != a->TransKeys().end();
             ++it) {
            if (trans) {
...
void SpliceKeys(
    ...
    if (start < 0.0f || end > lastFrame)
        MILO_WARN("%s has keyframes outside (0, %f)", anim1->Name(), lastFrame);
    else {
...
void RndScaleObject(Hmx::Object *o, float f1, float f2) {
    ...
    if (meshanim) {
        if (meshanim->KeysOwner() == meshanim) {
            for (Keys<std::vector<Vector3>, std::vector<Vector3> >::iterator it =
...
void SortXfms(RndMultiMesh *, const Vector3 &) { MILO_ASSERT(0, 3150); }
...
const char *CacheResource(const char *cc, CacheResourceResult &res) {
    ...
    if (stricmp(ext, "bmp") != 0 && stricmp(ext, "png") != 0) {
        ...
        if (movieExt) {
            return MakeString(
                "%s/%s.%s", FileGetPath(localized, 0), FileGetBase(localized, 0), movieExt
            );
        } else {
        ...
        strcpy(
            ...
            MakeString(
                "%s/gen/%s.%s_%s",
                FileGetPath(localized, 0),
                FileGetBase(localized, 0),
...
void RndUtlInit() {
    if (!UsingCD()) {
        sSphereDir = DirLoader::LoadObjects(
            FilePath(FileSystemRoot(), "rndobj/sphere.milo"), 0, 0
        );
    }
    if (sSphereDir) {
...
DECOMP_FORCEACTIVE(
    Utl,
    "%d animated meshes reset",
    "%d tex xfms collapsed",
    "%d rendered textures reset",
    "%d items deleted from unused buffers",
    "%d materials converted to Src\n%d materials converted to Add",
    "%d objects reordered",
    "Camera Y Field of View is 0"
)
...
// fn_806598F4
void ListDrawGroups(
    RndDrawable *draw, class ObjectDir *dir, std::list<RndGroup *> &gList
) {
