...
void RndCam::ScreenToWorld(const Vector2 &v2, float f, Vector3 &vout) const {
    float ynorm = (v2.y - mScreenRect.y) / mScreenRect.h;
    float xnorm = (v2.x - mScreenRect.x) / mScreenRect.w;
    vout.z = f;
    vout.y = (ynorm * 2.0f - 1.0f) * f;
    vout.x = (xnorm * 2.0f - 1.0f) * f;
...
void RndCam::Load(BinStream &bs) {
    ...
    if (rev == 8) {
        ObjPtrList<Hmx::Object, class ObjectDir> d(this, kObjListNoNull);
        int x;
        bs >> x >> d;
    }
...
RndCam::RndCam()
    : mNearPlane(1.0f), mFarPlane(1000.0f), mYFov(0.6024178f), mUnknownFloat(1.0f),
      mZRange(0.0f, 1.0f), mScreenRect(0.0f, 0.0f, 1.0f, 1.0f), mTargetTex(this, 0) {
    UpdateLocal();
}
