...
BinStream &
WriteChunks(BinStream &bs, const void *data, int total_len, int max_chunk_size) {
    int curr_size = 0;
    while (curr_size != total_len) {
        int len_left = Min(total_len - curr_size, max_chunk_size);
...
ChunkStream::ChunkStream(
    ...
    : BinStream(false), mFilename(file), mType(type), mChunkInfo(compress),
      mIsCached(cached), mStartTime(), mRecommendedChunkSize(chunkSize),
...
void ChunkStream::SeekImpl(int offset, BinStream::SeekType s) {
    if (s == kSeekCur && offset >= 0) {
        ...
        MILO_FAIL("Can only seek forward from current position on chunkstream");
}
...
DECOMP_FORCEACTIVE(ChunkStream, "mCurBufOffset == (*mCurChunk & kChunkSizeMask)")
...
void ChunkStream::DecompressChunk(DecompressTask &task) {
    ...
    if (id == 0xCDBEDEAF) {
        ...
        char *dataOffset = &task.out_data[task.mOutLen] - dataMsk;
        DecompressMem(
            dataOffset + 10, dataMsk - 0x12, task.out_data, out_len, false, task.mFilename
        );
    } else {
        MILO_ASSERT(task.mID == CHUNKSTREAM_Z_ID, 977);
        char *dataOffset = &task.out_data[task.mOutLen];
        DecompressMem(
            dataOffset - dataMsk, dataMsk, task.out_data, out_len, false, task.mFilename
        );
    }
