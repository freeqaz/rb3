...
float DisplayEvents(DataEventList *events, float f1, float f2) {
    ...
    while (min < events->Size() && events->Event(min).start < fsum) {
        ...
        float start14c = 5.0f + (curEvent.start - f2) * 200.0f;
        ...
        TheRnd->DrawRect(
            Hmx::Rect(start14c, f9, Max(1.0f, start150 - start14c), 12.0f),
            Hmx::Color(0, 0, 1),
            0,
            0,
            0
    ...
    TheRnd->DrawRect(
        Hmx::Rect(5.0f, f9 - 2.0f, 1.0f, 14.0f), Hmx::Color(1, 0, 0), 0, 0, 0
    );
    return f9 + f10;
}