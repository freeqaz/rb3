...
MeterDisplay::MeterDisplay()
    : mMeterAnim(0), mAnimPeriod(0), unk114(0), unk118(-1), mMeterLabel(0), mShowText(0),
      mPercentageText(0), mHideDenominator(0), mWrapperText(gNullStr), mCurrentValue(0),
      mMaxValue(0) {}

MeterDisplay::~MeterDisplay() { delete mMeterLabel; }
...
void MeterDisplay::Update() {
    ...
    if (typeDef->FindData(meter_label, label, false)) {
        ...
        if (!mMeterLabel)
            mMeterLabel = Hmx::Object::New<BandLabel>();
