...
void StreakMeter::SyncObjects() {
    ...
    if (mPartBarsGroup) {
        ...
        for (int i = 0; i < 3; i++) {
            mPartColorAnims.push_back(ObjPtr<RndPropAnim, ObjectDir>(
                this, Find<RndPropAnim>(MakeString("part_color%d.anim", i + 1), true)
            ));
            mPartFadeAnims.push_back(ObjPtr<RndPropAnim, ObjectDir>(
                this, Find<RndPropAnim>(MakeString("part_fade%d.anim", i + 1), true)
            ));
            mPartWipeAnims.push_back(ObjPtr<RndPropAnim, ObjectDir>(
                this, Find<RndPropAnim>(MakeString("part_wipe%d.anim", i + 1), true)
            ));
            mPartWipeResidualAnims.push_back(ObjPtr<RndPropAnim, ObjectDir>(
                this,
                Find<RndPropAnim>(MakeString("part_wipe%d_residual.anim", i + 1), true)
            ));
            mPartSparksLaunchers.push_back(ObjPtr<RndPartLauncher, ObjectDir>(
