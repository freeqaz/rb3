 ...
 *
 */
#ifndef _STLP_VECTOR_SIZED_C
#define _STLP_VECTOR_SIZED_C

#if !defined (_STLP_INTERNAL_VECTOR_SIZED_H)
...
#else
#  define __iterator__        _STLP_TYPENAME_ON_RETURN_TYPE _VECTOR_IMPL<_Tp, _Size, _Alloc>::iterator
#endif
...
template <class _Tp, class _Size, class _Alloc>
void _VECTOR_IMPL<_Tp, _Size, _Alloc>::reserve(size_type __n) {
  if (capacity() < __n) {
    if (max_size() < __n) {
...
template <class _Tp, class _Size, class _Alloc>
void _VECTOR_IMPL<_Tp, _Size, _Alloc>::_M_insert_overflow_aux(pointer __pos, const _Tp& __x, const __false_type& /*NOT TO USE!!*/,
...
template <class _Tp, class _Size, class _Alloc>
void _VECTOR_IMPL<_Tp, _Size, _Alloc>::_M_insert_overflow(pointer __pos, const _Tp& __x, const __true_type& /*_TrivialCpy*/,
                                                   size_type __fill_len, bool __atend ) {
  const size_type __old_size = size();
  size_type __len = __old_size + (max)(__old_size, __fill_len);
...
template <class _Tp, class _Size, class _Alloc>
_VECTOR_IMPL<_Tp, _Size, _Alloc>&
_VECTOR_IMPL<_Tp, _Size, _Alloc>::operator=(const _VECTOR_IMPL<_Tp, _Size, _Alloc>& __x) {
  if (&__x != this) {
    const size_type __xlen = __x.size();
    if (__xlen > capacity()) {
      pointer __tmp = _M_allocate_and_copy(__xlen, __CONST_CAST(const_pointer, __x._M_ptr._M_data)+0, __CONST_CAST(const_pointer, __x._M_finish())+0);
      _M_clear();
      this->_M_ptr._M_data = __tmp;
      _M_set_data_size(__xlen);
    } else if (size() >= __xlen) {
      pointer __i = __copy_ptrs(__CONST_CAST(const_pointer, __x._M_ptr._M_data)+0, __CONST_CAST(const_pointer, __x._M_finish())+0, this->_M_ptr._M_data, _TrivialAss());
      _STLP_STD::_Destroy_Range(__i, this->_M_finish());
    } else {
      __copy_ptrs(__CONST_CAST(const_pointer, __x._M_ptr._M_data), __CONST_CAST(const_pointer, __x._M_ptr._M_data) + size(), this->_M_ptr._M_data, _TrivialAss());
      __uninitialized_copy(__CONST_CAST(const_pointer, __x._M_ptr._M_data) + size(),
...
template <class _Tp, class _Size, class _Alloc>
void _VECTOR_IMPL<_Tp, _Size, _Alloc>::_M_fill_assign(size_t __n, const _Tp& __val) {
  if (__n > capacity()) {
    _VECTOR_IMPL<_Tp, _Size, _Alloc> __tmp(__n, __val, get_allocator());
    __tmp.swap(*this);
  } else if (__n > size()) {
...
template <class _Tp, class _Size, class _Alloc>
__iterator__
_VECTOR_IMPL<_Tp, _Size, _Alloc>::insert(iterator __pos, const _Tp& __x) {
