...
ArkFile::ArkFile(const char *iFilename, int iMode)
    : mNumOutstandingTasks(0), mBytesRead(0), mTell(0), mFail(0), mReadAhead(true),
      mFilename(iFilename) {
    bool fileinfores = TheArchive->GetFileInfo(
        FileMakePath(".", iFilename, 0), mArkfileNum, mByteStart, mSize, mUCSize
    );
...
int ArkFile::Tell() { return mTell; }

bool ArkFile::Eof() { return (mSize - mTell) == 0; }

bool ArkFile::Fail() { return (mFail != 0); }

int ArkFile::Size() { return mSize; }

int ArkFile::UncompressedSize() { return mUCSize; }
