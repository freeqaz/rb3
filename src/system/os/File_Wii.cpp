...
bool FileIsLocal(const char *file) { return FileIsDLC(file); }
...
int FileMkDir(const char *) { return -1; }

void FileEnumerate(
    const char *c1, void (*v)(const char *, const char *), bool b1, const char *c2, bool b2
) {
    if (UsingCD())
        TheArchive->Enumerate(c1, v, b1, c2);
    else
        HolmesClientEnumerate(c1, v, b1, c2, b2);
}
