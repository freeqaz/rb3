...
HomeMenu::HomeMenu()
    : mAllowHomeMenu(false), mHomeMenuActive(false), mSoundActive(true), mInGame(false),
      mForcedHomeMenu(false), unk_0x9(false), unk_0xA(false), mHomeMenuLocked(0),
      unk_0x10(5), mBanIconActive(false), mBanIconMode(0), mBanIconTime(0), mIconTpl(0),
      mBanIconAlpha(0), mSDIconActive(0), mSDIconMode(0), mSDIconTime(0), mSDIconTpl(0),
      mSDIconAlpha(0), unk_0x38(0), unk_0x44(1) {}

void HomeMenu::PreInit() {
    if (!mIconTpl) {
        mIconTpl = (TPLPalette *)ReadDvdFile(
            MakeString("%s/HomeMenu/homeBtnIcon.tpl", FileSystemRoot()), 0
        );
...
void HomeMenu::Init() {
    ...
    PreInit();
    mSDIconTpl = (TPLPalette *)ReadDvdFile(
        MakeString("%s/HomeMenu/SdCard.tpl", FileSystemRoot()), 0
    );
...
void HomeMenu::Begin() {
    ...
    HmbRsoInit();
    unk34 = (TPLPalette *)ReadDvdFile(
        MakeString("%s/HomeMenu/HomeButton%d/HomeButtonSe.arc", FileSystemRoot(), 2), 0
    );
...
void HomeMenu::AllowHomeMenu(bool allow) {
    if (mHomeMenuLocked == 0)
        mAllowHomeMenu = allow;
    else
        mAllowHomeMenu = false;
}
...
void HomeMenu::RegisterCallback(HomeMenu::Callback *cb) { mCallbacks.push_back(cb); }
