...
static DataNode OnFileExecRoot(DataArray *da) { return gExecRoot; }

static DataNode OnFileRoot(DataArray *da) { return gRoot; }

static DataNode OnFileGetDrive(DataArray *da) { return FileGetDrive(da->Str(1)); }

static DataNode OnFileGetPath(DataArray *da) { return FileGetPath(da->Str(1), 0); }

static DataNode OnFileGetBase(DataArray *da) { return FileGetBase(da->Str(1), 0); }

static DataNode OnFileGetExt(DataArray *da) { return FileGetExt(da->Str(1)); }

static DataNode OnFileMatch(DataArray *da) { return FileMatch(da->Str(1), da->Str(2)); }
...
void OnFrameRateRecurseCB(const char *cc1, const char *cc2) {
    ...
    str = str.substr(0, theStrLen - strlen(keepStr));
...
File *NewFile(const char *cc, int i) {
    ...
    if (!cc || !*cc)
        ...
        if (strstr(cc, "/band3_ng/")) {
            ...
            if (!ThePlatformMgr.AreSFXEnabled() && strstr(cc, ".wav"))
                return nullptr;
            else if (strstr(cc, "_norm."))
                return nullptr;
            else if (strstr(cc, "_spec."))
                return nullptr;
            else {
                if (i & 2U) {
                    char buf[256];
...
const char *FileRelativePath(const char *root, const char *filepath) {
    ...
    for (const char *tok = strtok(rootBuf, "/"); tok != nullptr;
         tok = strtok(nullptr, "/")) {
        list218.push_back(tok);
    }
    for (const char *tok = strtok(fpBuf, "/"); tok != nullptr;
         tok = strtok(nullptr, "/")) {
