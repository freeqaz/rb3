...
WiiNetworkSocket::WiiNetworkSocket(bool streaming) : mStreaming(streaming), mFail(0) {
    Init();
    if (!mStreaming)
        mSocket = SOSocket(PF_INET, SOCK_DGRAM, PROTO_DGRAM_TCP);
    else
        mSocket = SOSocket(PF_INET, SOCK_STREAM, PROTO_DGRAM_TCP);
    mFail = mSocket < SO_OK;
    if (!mFail) {
...
WiiNetworkSocket::WiiNetworkSocket(int socket, bool streaming)
    : mSocket(socket), mStreaming(streaming), mFail(0) {
    int ret = SOFcntl(mSocket, 3, 0);
    SOFcntl(mSocket, 4, ret | 4);
}

WiiNetworkSocket::~WiiNetworkSocket() { Disconnect(); }

int NetworkSocket::IPStringToInt(const String &str) {
    WiiNetworkSocket::Init();
    u32 addr;
    if (SOInetAtoN(str.c_str(), &addr) != 0)
        return addr;
    return -1;
}
...
int WiiNetworkSocket::Listen() { return SOListen(mSocket, 5); }
...
bool WiiNetworkSocket::CanSend() const {
    ...
    if (SOPoll(&polls, 1, 0) >= 0 && polls.result == polls.mask)
        ret = true;
    return ret;
}
