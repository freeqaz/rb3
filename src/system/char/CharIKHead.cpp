...
CharIKHead::CharIKHead()
    : mPoints(this), mHead(this), mSpine(this), mMouth(this), mTarget(this),
      mHeadFilter(0.0f, 0.0f, 0.0f), mTargetRadius(0.75f), mHeadMat(0.5f), mOffset(this),
      mOffsetScale(1.0f, 1.0f, 1.0f), mUpdatePoints(1), mMe(this) {}
...
void CharIKHead::Highlight() {
    ...
    if (weight == 0 || !mHead || !mTarget || !mSpine)
        ...
        for (int i = 1; i < mPoints.size(); i++) {
            ...
            TheRnd->DrawLine(
                mPoints[i].unkc, mPoints[i - 1].unkc, Hmx::Color(0, 1, 0), false
            );
        }
    }
#endif
