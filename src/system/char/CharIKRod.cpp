...
bool CharIKRod::ComputeRod(Transform &tf) {
    ObjPtr<RndTransformable> &_ref0 = mLeftEnd;
    if (mDest == 0 || _ref0 == 0 || mRightEnd == 0)
        return false;
    Interp(_ref0->WorldXfm().v, mRightEnd->WorldXfm().v, mDestPos, tf.v);
    if (mVertical)
        tf.m.x.Set(0.0f, 0.0f, -1.0f);
    else {
        Interp(_ref0->WorldXfm().m.x, mRightEnd->WorldXfm().m.x, mDestPos, tf.m.x);
        Normalize(tf.m.x, tf.m.x);
    }
    if (mSideAxis)
        tf.m.z = mSideAxis->WorldXfm().m.z;
    else
        Subtract(_ref0->WorldXfm().v, mRightEnd->WorldXfm().v, tf.m.z);
