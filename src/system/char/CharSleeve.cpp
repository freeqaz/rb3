...
CharSleeve::CharSleeve()
    : mSleeve(this), mTopSleeve(this), mPos(0, 0, 0), mLastPos(0, 0, 0), mLastDT(0.0f),
      mInertia(0.5f), mGravity(1.0f), mRange(0.0f), mNegLength(0.0f), mPosLength(0.0f),
...
void CharSleeve::Poll() {
    if (mSleeve && mSleeve->TransParent()) {
        ...
        if (mMe && mMe->Teleported()) {
            ...
            Vector3 va8;
            ScaleAdd(
                sleeveparent->WorldXfm().v, sleeveparent->WorldXfm().m.x, dotted, va8
            );
