...
int FileMerger::FindMergerIndex(Symbol name, bool warn) {
    ...
    if (warn)
        MILO_WARN("%s could not find Merger %s", PathName(this), name);
    return -1;
}
        ...
        if (o1 == loaderDir) {
            ...
            if (strnicmp("spot_", o1Name, 5) == 0 || strnicmp("bone_", o1Name, 5) == 0) {
                ...
                if (dynamic_cast<RndGroup *>(o2) || dynamic_cast<CharClipGroup *>(o2)
                    ...
                    return kMerge;
                else if (dynamic_cast<RndMat *>(o2))
                    return kKeep;
                else {
...
MergeFilter::Action FileMerger::FilterSubdir(ObjectDir *o1, ObjectDir *o2) {
    ...
    if (mFilter)
        ...
        a = DefaultSubdirAction(o1, (Subdirs)merger->mSubdirs);
...
void FileMergerObjPtrSave(BinStream &bs, const ObjPtr<ObjectDir> &ptr) { bs << ptr; }
...
BinStream &operator>>(BinStream &bs, FileMerger::Merger &m) {
    ...
    if (FileMerger::gRev != 0) {
        if (FileMerger::gRev != 4)
            bs >> m.mProxy;
        bs >> m.mSubdirs;
        if (FileMerger::gRev > 2)
            bs >> m.mPreClear;
    }
    return bs;
}
