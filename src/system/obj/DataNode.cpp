...
#ifdef MILO_DEBUG
DECOMP_FORCEACTIVE(
    DataNode, "Data %s is not Glob (file %s, line %d)", "Data %s is not Glob"
)
#endif
...
#ifdef MILO_DEBUG
DECOMP_FORCEACTIVE(
    DataNode, "Data %s is not Property (file %s, line %d)", "Data %s is not Property"
)
#endif
...
bool DataNode::operator==(const DataNode &node) const {
    if (mType == node.mType) {
        ...
        if (mType == kDataObject) {
            if (mValue.object != 0)
                obj1 = mValue.object->Name();
            else
                obj1 = "";
            obj2 = node.LiteralStr();
        } else {
            obj1 = LiteralStr();
            if (node.mValue.object != 0)
                obj2 = node.mValue.object->Name();
            else
                obj2 = "";
        }
