...
void TypeProps::InsertArrayValue(
            ...
            if (def) {
                ...
                while (mMap && i < mMap->Size()) {
                    ...
                    if (theArr) {
                        ...
                        if (node.Type() == kDataCommand)
                            goto next;
                        if (theArr->Node(1).CompatibleType(
                                CONST_ARRAY(mMap)->Node(i + 1).Type()
                            ))
                            goto next;
                    }
    ...
    if (def) {
        if (mMap && LOADMGR_EDITMODE) {
            for (int i = 0; mMap && i < mMap->Size(); i += 2) {
                ...
                if (!keyArr
                    || (CONST_ARRAY(keyArr)->Node(1).Type() != kDataCommand)
                        && !keyArr->Node(1).CompatibleType(
                            CONST_ARRAY(mMap)->Node(i + 1).Type()
                        )) {
                    MILO_LOG(
