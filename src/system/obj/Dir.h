...
public:
    ...
    ObjDirPtr(const ObjDirPtr &dPtr) : mDir(dPtr.mDir), mLoader(0) {
        if (mDir)
            mDir->AddRef(this);
    }
    ...
    FilePath &GetFile() const {
        ...
        if (mLoader)
            return mLoader->mFile;
        if (mDir)
            return mDir->mStoredFile;
    ...
    void LoadFile(const FilePath &p, bool async, bool share, LoaderPos pos, bool b3) {
        *this = 0;
        ...
        if (!d) {
            ...
            if (!p.empty())
                d = new DirLoader(p, pos, 0, 0, 0, b3);
        }
        mLoader = d;
        if (d) {
            ...
            MILO_WARN("Couldn't load %s", p);
    }

    bool operator==(const ObjDirPtr<T> &dPtr) const { return mDir == dPtr.mDir; }
...
template <class T1>
BinStream &operator>>(BinStream &bs, ObjDirPtr<T1> &ptr) {
    FilePath fp;
    bs >> fp;
...
public:
     ...
     * @param [in] add If true, and there isn't an Entry for this Object in this Dir, add
     * it.
    ...
    DELETE_OVERLOAD;
    NEW_OBJ(ObjectDir)
    static void Register() { REGISTER_OBJ_FACTORY(ObjectDir) }
    ...
    bool mIsSubDir; // 0x54
    /** "How is this inlined as a subdir?  Note that when you change this, you must resave
     * everything subdiring this file for it to take effect" */
