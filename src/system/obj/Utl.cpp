...
static int gCopyMax;
static long long gOldSkoolResults[0x4000];
static long long gWGPResults[0x4000];
static long long gNoWGPResults[0x4000];
...
const char *PathName(const Hmx::Object *o) {
    if (o)
        return ((Hmx::Object *)o)->FindPathName();
    else
        return "NULL Object";
}
...
DECOMP_FORCEACTIVE(Utl, "Object", "ext", ".")
                ...
                if (toIt != toProps.end()) {
                    if (*toIt == prop) {
                        ...
                        if (fromType == toType) {
                            ...
                            if (!fromVal->Sym().Null()) {
                                Hmx::Object *objProp =
                                    from->Dir()->FindObject(fromVal->Sym().mStr, false);
                                if (objProp) {
...
void MergeObject(Hmx::Object *o1, Hmx::Object *o2, ObjectDir *dir, MergeFilter &filt) {
    if (o2 != o1)
        MergeObject(o1, o2, dir, filt.Filter(o1, o2, dir));
}
...
int SubDirStringUsed(ObjectDir *dir) {
    if (!dir)
        ...
        for (std::vector<ObjDirPtr<ObjectDir> >::iterator it = dir->mSubDirs.begin();
             it != dir->mSubDirs.end();
             ++it) {
...
const char *NextName(const char *old_name, ObjectDir *dir) {
    ...
    char *ptr;
    for (ptr = (char *)base + strlen(base); (base < ptr && ((unsigned char)(ptr[-1] - 0x30) <= 9)); ptr--)
        ;
    int atoied = 0;
    if (*ptr != '\0')
        atoied = atoi(ptr);
