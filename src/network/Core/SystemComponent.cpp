...
namespace Quazal {
    ...
    bool SystemComponent::ValidTransition(_State state) {
        ...
        case Uninitialized:
            return (
                state == Initializing || state == TerminatingInUse || state == Terminated
            );
        case Initializing:
            return state == Ready;
        case Ready:
        ...
        case Terminating:
            return (mRefs == 0 && state == TerminatingInUse);
    ...
    SystemComponent::_State SystemComponent::Terminate() {
        ...
        case TerminatingInUse:
            ...
            return mState;
            break;
        default: { // necessary because the compiler interleaves the loading/returning
                   // elsewise
            TraceLog::ScopedIndent indent(2);
            TraceImpl(1);
        }
    ...
    SystemComponent::Use::Use(Quazal::SystemComponent *sc, const char *cc)
        : mComponent(sc), mName(cc) {
        bool b;
        if (sc->UseIsAllowed()) {
