...
namespace Quazal {
    Log::Log()
        : m_oLogDevice(new(__FILE__, 34) LogDeviceDebugOutput),
          m_oOutputFormat(new(__FILE__, 38) OutputFormat), unk_0xC(NULL) {}
    ...
    void Log::OutputImpl(const char *cc, va_list l, void *v) {
        char buf[0x1000];
        if (m_oLogDevice == NULL)
            return;
        if (unk_0xC != NULL && !unk_0xC->AcquireRef())
            return;
        if (m_oOutputFormat == NULL)
            return;
